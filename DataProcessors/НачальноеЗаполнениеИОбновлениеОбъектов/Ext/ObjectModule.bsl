...
Функция СформированиеЗначениеСсылочногоТипа(ТипРеквизита, ЗначениеРеквизита, СоздаватьЭлементыСправочников = Ложь)
	...
	Если Значение = Неопределено Тогда
		Если НЕ ПустаяСтрока(ЗначениеРеквизита) Тогда
			Сообщить(Локализация.СтрШаблон(НСтр("ru='Не удалось получить значение ¤1¤ по типу ¤2¤!';uk='Не вдалося одержати значення ¤1¤ по типу ¤2¤!'"), ЗначениеРеквизита, ТипРеквизита));
		КонецЕсли;
		Возврат ЗначениеРеквизита;
	Иначе
		Возврат Значение;
...
Процедура ЗаполнитьСправочник(ИмяОбъекта)
	...
	Для Сч = 4 по ЧислоРеквизитов Цикл
		...
		Если МетаданныеРеквизита = Неопределено Тогда
			Сообщить(Локализация.СтрШаблон(НСтр("ru='Ошибка при заполнении объекта ¤1¤! Не найден рекизит ¤2¤."
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(Локализация.СтрШаблон(НСтр("ru='Не найден предопределённый элемент ¤1¤ для справочника ¤2¤';uk='Не знайдений визначений елемент ¤1¤ для довідника ¤2¤'"), Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			МетаданныеРеквизита = МетаданныеСправочника.Реквизиты.Найти(ИмяРеквизита);
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(Локализация.СтрШаблон(НСтр("ru='Не найден реквизит ¤1¤ справочника ¤2¤!';uk='Не знайдений реквізит ¤1¤ довідника ¤2¤!'"), ИмяРеквизита, ИмяОбъекта));
				Продолжить;
			КонецЕсли;
		...
		Если ФлагИзменений Тогда
			...
			Если МетаданныеСправочника.ДлинаКода > 0 И МетаданныеСправочника.КонтрольУникальности = Истина Тогда
				...
				Если НЕ КодЭлементаУникален("Справочник."+ИмяОбъекта,ТекущаяЗапись.Код,СсылкаНаЭлемент) Тогда
					...
					Сообщить(Локализация.СтрШаблон(НСтр("ru='Не удалось записать объект справочника ¤1¤: ¤2¤, по причине того, что сгенерированный код был не уникален!';uk=""Не вдалося записати об'єкт довідника ¤1¤: ¤2¤, через те, що згенерований код був не унікальний!"""), ИмяОбъекта, ТекущаяЗапись));
					ФлагИзменений = Ложь;
				КонецЕсли;
			Конецесли;
			Если ФлагИзменений Тогда
				...
				Исключение
					Сообщить(Локализация.СтрШаблон(НСтр("ru='Не удалось записать объект справочника ¤1¤: ¤2¤!';uk=""Не вдалося записати об'єкт довідника ¤1¤: ¤2¤!"""), ИмяОбъекта, ТекущаяЗапись));
				КонецПопытки;	
			КонецЕсли;	
		КонецЕсли;	
...
Процедура ЗаполнитьРегистрСведений(ИмяОбъекта)
	...
	Для Сч = 1 по ЧислоИзмерений Цикл
		...
		Если МетаданныеИзмерения = Неопределено Тогда
			Сообщить(Локализация.СтрШаблон(НСтр("ru='Ошибка при заполнении объекта ¤1¤! Не найдено измерение ¤2¤."
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяИзмерения));
	...
	Для Сч = 1 по ЧислоРесурсов Цикл
		...
		Если МетаданныеРесурса = Неопределено Тогда
			Сообщить(Локализация.СтрШаблон(НСтр("ru='Ошибка при заполнении объекта ¤1¤! Не найден ресурс ¤2¤."
...
"Об'єкт не заповнений!"""), ИмяОбъекта, МетаданныеРесурса));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Исключение
			...
			Сообщить(Локализация.СтрШаблон(НСтр("ru='В регистре сведений ¤1¤ не удалось создать запись с параметрами ¤2¤';uk='В регістрі відомостей ¤1¤ не вдалося створити запис із параметрами ¤2¤'"), ИмяОбъекта, ПараметрыЗаписи));
			Продолжить;
		КонецПопытки;	
...
Процедура ЗаполнитьТабличнуюЧасть(ТекущаяЗапись, Имя, ИмяТЧ, МакетТЧ, ТаблицаРеквизитовТЧ, МетаданныеТЧ, ФлагИзменений)
	...
	Для НомСтроки = 1 по ЧислоСтрок  Цикл
		...
		Для Сч = 1 по ЧислоРеквизитов Цикл
			...
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(Локализация.СтрШаблон(НСтр("ru='Не найден реквизит ¤1¤ табличной части ¤2¤!';uk='Не знайдений реквізит ¤1¤ табличної частини ¤2¤!'"), ИмяРеквизита, ИмяТЧ));
				Продолжить;
			КонецЕсли;
    ...
    Если ЕстьТЧ Тогда
		...
		Для Каждого Область из КоллекцияОбластей Цикл
			...
			Если ЕстьНужнаяТЧ = Неопределено Тогда
				Сообщить(Локализация.СтрШаблон(НСтр("ru='Ошибка при заполнении объекта ¤1¤! Не найдена ТЧ ¤2¤!';uk=""Помилка при заповненні об'єкта ¤1¤! Не знайдена ТЧ ¤2¤!"""), ИмяОбъекта, ИмяТч));
				Возврат;
			КонецЕсли;
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(Локализация.СтрШаблон(НСтр("ru='Не найден предопределённый элемент ¤1¤ для ПВХ ¤2¤';uk='Не знайдений визначений елемент ¤1¤ для ПВХ ¤2¤'"), Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
		...
		Если ФлагИзменений Тогда
			...
			Попытка
				ТекущаяЗапись.Записать();
			Исключение
				Сообщить(Локализация.СтрШаблон(НСтр("ru='Не удалось записать объект ПВХ ¤1¤: ¤2¤!';uk=""Не вдалося записати об'єкт ПВХ ¤1¤: ¤2¤!"""), ИмяОбъекта, ТекущаяЗапись));
			КонецПопытки;	
		КонецЕсли;	
	КонецЦикла;	
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(Локализация.СтрШаблон(НСтр("ru='Не найден предопределённый элемент ¤1¤ для плана счетов ¤2¤';uk='Не знайдений визначений елемент ¤1¤ для плану рахунків ¤2¤'"), Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			МетаданныеРеквизита = МетаданныеПС.Реквизиты.Найти(ИмяРеквизита);
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(Локализация.СтрШаблон(НСтр("ru='Не найден реквизит ¤1¤ плана счетов ¤2¤!';uk='Не знайдений реквізит ¤1¤ плану рахунків ¤2¤!'"), ИмяРеквизита, ИмяОбъекта));
				Продолжить;
			КонецЕсли;
		...
		Если ФлагИзменений Тогда
			...
			Если МетаданныеПС.ДлинаКода > 0 И МетаданныеПС.КонтрольУникальности = Истина Тогда
				...
				Если НЕ КодЭлементаУникален("ПланСчетов."+ИмяОбъекта,ТекущаяЗапись.Код,СсылкаНаЭлемент) Тогда
					// всеравно есть лишние проблемы :( не будем записывать
					Сообщить(Локализация.СтрШаблон(НСтр("ru='Не удалось записать объект плана счетов ¤1¤: ¤2¤, по причине того, что сгенерированный код был не уникален!';uk=""Не вдалося записати об'єкт плану рахунків ¤1¤: ¤2¤, через те, що згенерований код був не унікальний!"""), ИмяОбъекта, ТекущаяЗапись));
					ФлагИзменений = Ложь;
				КонецЕсли;
			Конецесли;
			Если ФлагИзменений Тогда
				Попытка
					ТекущаяЗапись.Записать();
				Исключение
					Сообщить(Локализация.СтрШаблон(НСтр("ru='Не удалось записать объект плана счетов ¤1¤: ¤2¤!';uk=""Не вдалося записати об'єкт плану рахунків ¤1¤: ¤2¤!"""), ИмяОбъекта, ТекущаяЗапись));
				КонецПопытки;	
			КонецЕсли;	
		КонецЕсли;	
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(Локализация.СтрШаблон(НСтр("ru='Не найден предопределённый элемент ¤1¤ для плана видов расчета ¤2¤';uk='Не знайдений визначений елемент ¤1¤ для плану видів розрахунку ¤2¤'"), Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			МетаданныеРеквизита = МетаданныеПВР.Реквизиты.Найти(ИмяРеквизита);
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(Локализация.СтрШаблон(НСтр("ru='Не найден реквизит ¤1¤ плана видов расчета ¤2¤!';uk='Не знайдений реквізит ¤1¤ плану видів розрахунку ¤2¤!'"), ИмяРеквизита, ИмяОбъекта));
				Продолжить;
			КонецЕсли;
			НовоеЗначение = СформироватьЗначение2(ТаблицаРеквизитов, ИмяРеквизита, ЗначениеРеквизита, Параметр);
			Если НовоеЗначение <> ТекущаяЗапись[ИмяРеквизита] Тогда
		...
		Если ФлагИзменений Тогда
			Попытка
				ТекущаяЗапись.Записать();
			Исключение
				Сообщить(Локализация.СтрШаблон(НСтр("ru='Не удалось записать объект план видов расчета ¤1¤: ¤2¤!';uk=""Не вдалося записати об'єкт план видів розрахунку ¤1¤: ¤2¤!"""), ИмяОбъекта, ТекущаяЗапись));
			КонецПопытки;	
		КонецЕсли;	
	КонецЦикла;	
