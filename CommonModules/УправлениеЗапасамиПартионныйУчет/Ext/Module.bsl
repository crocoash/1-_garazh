...
Процедура ЗаписатьДвиженияДокумента(СтруктураПараметров, ТаблицаСписания, Замещать = Истина) Экспорт
	...
	// Управленческий учет
	ЗаписатьДвиженияДокументаУпр(СтруктураПараметров, ТаблицаСписания, Замещать);   //СтруктураПараметров.ТаблицаДвиженийПартииТоваровНаСкладахУпр[0]
...
Процедура СписаниеПартий(СтрокаДокумента, СтруктураПараметров, РегистрУчета)
	...
	Парт = ПолучитьМассивПартийДляСписания(СтрокаДокумента);
	
	СтруктураСтоимости = РассчитатьСтоимостьПоступленияДляСписания(СтрокаДокумента);
	СтоимостьПоступление = СтруктураСтоимости.СтоимостьПоступление;
	КоличествоПоступление = СтруктураСтоимости.КоличествоПоступление;
	СтоимостьПоступлениеОсталосьПогасить = СтруктураСтоимости.СтоимостьПоступлениеОсталосьПогасить;
	
	ВыполнитьСписаниеПоПартиям(
		СтрокаДокумента,
		СтруктураПараметров,
		РегистрУчета,
		ИмяРегистра,
		СтрокаДереваПартий,
		СтруктураИзмерений,
		ТаблицаОстатковПартий,
		Парт,
		КоличествоОсталосьПогасить,
		СтоимостьПоступлениеОсталосьПогасить,
		КоличествоПоступление,
		СтоимостьПоступление,
		КоличествоСписанноеПоДоументуПартии
	);
	
КонецПроцедуры


Функция ПолучитьМассивПартийДляСписания(СтрокаДокумента) Экспорт
	
	Парт = Новый Массив;
	
	Попытка
		Док = СтрокаДокумента.Регистратор.Товары[СтрокаДокумента.НомерСтроки - 1];
		
		Если ТипЗнч(СтрокаДокумента.Регистратор) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
			
			ЗаказПокупателя = Док.ДокументРезерва;
			ЗаказПоставщику = СтрокаДокумента.Регистратор.ЗаказПоставщику;
			
			Если ЗначениеЗаполнено(ЗаказПокупателя) И НЕ ЗначениеЗаполнено(ЗаказПоставщику) Тогда
				...
				Запрос.УстановитьПараметр("Момент", СтрокаДокумента.Регистратор.Дата);
				Запрос.УстановитьПараметр("Номенклатура", Док.Номенклатура);
				Запрос.УстановитьПараметр("Организация", СтрокаДокумента.Регистратор.Организация);
				Запрос.УстановитьПараметр("Склад", СтрокаДокумента.Регистратор.СкладОтправитель);
				
				ТЗПартии = Запрос.Выполнить().Выгрузить();
				Для Каждого СтрПартия Из ТЗПартии Цикл
					Если ПроверитьЗанятаЛиПартияДругимЗаказомПокупателя(СтрПартия.Партия, ЗаказПокупателя, Док.Номенклатура, СтрокаДокумента.Регистратор.СкладОтправитель) Тогда
						Продолжить;
					КонецЕсли;
					Парт.Добавить(СтрПартия.Партия);
				КонецЦикла;
				
			ИначеЕсли ЗначениеЗаполнено(ЗаказПокупателя) Или ЗначениеЗаполнено(ЗаказПоставщику) Тогда
				
				Запрос = Новый Запрос("ВЫБРАТЬ
				|	РазмещениеЗаказовПокупателей.Регистратор КАК ДокПоступление
				|ИЗ
				...
				|	И РазмещениеЗаказовПокупателей.ЗаказПоставщику = &ЗаказПоставщику
				|	И РазмещениеЗаказовПокупателей.Номенклатура = &Номенклатура
				|	И РазмещениеЗаказовПокупателей.Регистратор ССЫЛКА Документ.ПоступлениеТоваровУслуг");
				...
				Выборка = Запрос.Выполнить().Выбрать();
				Пока Выборка.Следующий() Цикл
					Парт.Добавить(Выборка.ДокПоступление);
				КонецЦикла;
				
				Если Парт.Количество() = 0 Тогда
					Запрос = Новый Запрос("ВЫБРАТЬ
					...
					+ ?(ЗначениеЗаполнено(ЗаказПоставщику), " И ПоступлениеТоваровУслугТовары.Ссылка.Сделка = &Сделка ", " ")
					+ "
					|	И ПоступлениеТоваровУслугТовары.Номенклатура = &Номенклатура
					|	И ПоступлениеТоваровУслугТовары.Заказ = &Заказ
					...
					Выборка = Запрос.Выполнить().Выбрать();
					Пока Выборка.Следующий() Цикл
						Парт.Добавить(Выборка.Партия);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
			
		Иначе
			Парт.Добавить(Док.ДокументПартии);
		КонецЕсли;
...
Функция РассчитатьСтоимостьПоступленияДляСписания(СтрокаДокумента) Экспорт
	
	Результат = Новый Структура;
	
	СтоимостьПоступление = СтрокаДокумента.СтоимостьПоступление;
	...
	СовпадаетСсуммы = Истина; 
	
	Если ЗначениеЗаполнено(СтрокаДокумента.ЗаказСписания) и СтрокаДокумента.ЗаказСписания.Проведен  Тогда
		СтрокаЗаказа = СтрокаДокумента.ЗаказСписания.Товары.Найти(СтрокаДокумента.Номенклатура, "Номенклатура"); 
		Если СтрокаЗаказа <> Неопределено Тогда
			СовпадаетСсуммы = (СтрокаЗаказа.БП_СуммаПоУсловиюПоставки = СтрокаЗаказа.Сумма);
		КонецЕсли;
		
	КонецЕсли;
	
	Если СтрокаЗаказа <> Неопределено И Не СовпадаетСсуммы 
		И СтрокаЗаказа.БП_СуммаПоУсловиюПоставки <> Неопределено Тогда
		
		СтоимостьПоступление = СтрокаЗаказа.БП_СуммаПоУсловиюПоставки;
		КоличествоПоступление = СтрокаЗаказа.Количество;
		
		Если СтрокаДокумента.ОтражатьВУправленческомУчете Тогда
			Если ЗначениеЗаполнено(КоличествоПоступление) И КоличествоПоступление <> 0 Тогда
				СтоимостьПоступлениеОсталосьПогасить = Окр(
					СтоимостьПоступление * СтрокаДокумента.Количество / КоличествоПоступление,
					2,
					1
				);
			ИначеЕсли ЗначениеЗаполнено(СтрокаДокумента.Количество) И СтрокаДокумента.Количество <> 0 Тогда
				СтоимостьПоступлениеОсталосьПогасить = Окр(СтоимостьПоступление, 2, 1);
			КонецЕсли;
		КонецЕсли;
	...
	Результат.Вставить("СтоимостьПоступление", СтоимостьПоступление);
	Результат.Вставить("КоличествоПоступление", КоличествоПоступление);
	Результат.Вставить("СтоимостьПоступлениеОсталосьПогасить", СтоимостьПоступлениеОсталосьПогасить);
...
Процедура ВыполнитьСписаниеПоПартиям(
	СтрокаДокумента,
	СтруктураПараметров,
	РегистрУчета,
	ИмяРегистра,
	...
	Если СтрокаДереваПартий <> Неопределено Тогда
		...
		Для Каждого СтрокаПартииРаспределения Из СтрокаДереваПартий.Строки Цикл
			...
			Если Парт.Количество() Тогда
				Если Парт.Найти(СтрокаПартии.ДокументОприходования) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СтрокаДокумента.ДокументПартии)
				И (СтрокаПартии.ДокументОприходования = СтрокаДокумента.ДокументПартии) Тогда
			...
			Если ЗначениеЗаполнено(КоличествоПоступление) И КоличествоПоступление <> 0 И СтоимостьПоступление <> 0 Тогда
				Движение.Стоимость = СтоимостьПоступление * Движение.Количество / КоличествоПоступление; // Окр( , 2, 1)
			Иначе
				Движение.Стоимость = СтрокаПартии.Стоимость * КоэффСписания; // Окр(, 2, 1)
			КонецЕсли;
...
Функция ПроверитьЗанятаЛиПартияДругимЗаказомПокупателя(ДокПартия, ЗаказПокупателя, Номенклатура, Склад)
	...
	ПартияЗанята = ТЗ.Количество();
	
	Если ПартияЗанята тогда
		Возврат ПартияЗанята;
	КонецЕсли;
	
	//перевірка чи зайнята партія з одного заказа покупця іншим документом замовлення постачальника
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	РегистрНакопления.РазмещениеЗаказовПокупателей КАК РазмещениеЗаказовПокупателей
	                      |ГДЕ
	                      |	РазмещениеЗаказовПокупателей.Номенклатура = &Номенклатура
	                      |	И РазмещениеЗаказовПокупателей.ЗаказПокупателя = &ЗаказПокупателя
	                      |	И РазмещениеЗаказовПокупателей.Регистратор = &ДокПартия");
...
Процедура ВыполнитьСписание(СтруктураПараметров, ТаблицаСписания, МоментКон, Останавливаться = Ложь, ПроведениеОстановлено = Ложь )
	...
	//СтруктураПараметров.ДеревоПартииТоваровНаСкладахУпр.Строки.Найти(СтрокаДокумента.НомерСтрокиДокумента, "НомерСтрокиДокумента").Строки[0]
	...
	//+
	Если СтруктураПараметров.Отказ И ТипЗнч(ОбрабатываемыйДокумент) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда
		ОбщегоНазначения.СообщитьОбОшибке("Проведение по партиям остановлено ",СтруктураПараметров.Отказ);
		Останавливаться = Истина;
	...
	Если Не ОбрабатываемыйДокумент = Неопределено Тогда
		...
		ЗаписатьДвиженияДокумента(СтруктураПараметров, ТаблицаСписания, Истина);  // +++ ect СтруктураПараметров.ТаблицаДвиженийПартииТоваровНаСкладахУпр[0]
