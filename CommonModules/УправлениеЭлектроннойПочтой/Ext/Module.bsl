...
//
// Нужна там, где выборка всей пачки упала: POP3 отдаёт письма по одному запросу
// на сообщение, и сбой на любом из них отменяет весь результат. Здесь каждое
// письмо берётся отдельной попыткой, а проблемное описывается в журнале
// (событие «Почта.Получение»), чтобы его можно было найти в ящике руками.
//
// Параметры
//  ИнтернетПочта   - ИнтернетПочта, открытое соединение
//  УчетнаяЗапись   - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты
//  МассивЗаголовков - Массив ИнтернетПочтовоеСообщение, заголовки к получению
//
// Возвращаемое значение:
...
Функция ПолучитьПисьмаПоОдному(ИнтернетПочта, УчетнаяЗапись, МассивЗаголовков) Экспорт
	...
	ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Информация, , ,
		...
		+ " з " + МассивЗаголовков.Количество() + ", пропущено " + Сбойных + ".");
...
Функция ОписаниеПисьма(Заголовок) Экспорт
	...
	Попытка
		Описание = Описание + " Від: " + СокрЛП(Строка(Заголовок.Отправитель)) + ".";
	Исключение
	КонецПопытки;
