...
Процедура УстановитьТекстПисьма(ИнтернетПисьмо, ПисьмоОбъект, УстанавливатьВидТекстаПисьма = Истина) Экспорт
	...
	Исключение
			//С.Д.
			Сообщить("Ошибка считывание тегов картинок");
		КонецПопытки;
		// Разбор через COM-объект HtmlFile зависит от того, что установлено на
		// машине: на части серверов у документа нет метода tags, и тогда падало
		// всё получение почты из-за одного HTML-письма. Не вышло — берём исходный
		// HTML как есть: письмо сохранится, только картинки внутри могут не
		// подставиться.
		Попытка
			ПисьмоОбъект.ТекстПисьма = НовыйHTMLДокумент.all.tags("html").item(0).outerHTML;
		Исключение
			ПисьмоОбъект.ТекстПисьма = ХТМЛ_Текст;
			ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Предупреждение, , ,
				"HTML листа не розібрано, збережено як є: " + ОписаниеОшибки());
		КонецПопытки;
...
Функция ПолучитьДокументОбъектИзИнтернетПочтовогоСообщения(ИнтернетПисьмо, УчетнаяЗапись = Неопределено, мРазделительАдресов = ",", ИсключениеТранзакции = Ложь)
	...
	// Основание ищем по заголовкам цепочки: сначала In-Reply-To (на что ответили
	// напрямую), потом References с конца — там вся цепочка переписки. Второй шаг
	// нужен, когда прямого предшественника в базе нет: например, промежуточный
	// ответ менеджера ушёл из Outlook, а не из 1С. Тогда берём ближайшее по цепочке
	// письмо, которое в базе есть, — и переписка складывается в дерево, а не
	// в плоский список под первым письмом.
	Кандидаты = ИдентификаторыИзЗаголовкаПисьма(ИнтернетПисьмо, "In-Reply-To");
	Цепочка   = ИдентификаторыИзЗаголовкаПисьма(ИнтернетПисьмо, "References");
	Позиция   = Цепочка.Количество() - 1;
	Пока Позиция >= 0 Цикл
		Если Кандидаты.Найти(Цепочка[Позиция]) = Неопределено Тогда
			Кандидаты.Добавить(Цепочка[Позиция]);
		КонецЕсли;
		Позиция = Позиция - 1;
	КонецЦикла;
	
	Для Каждого ИдентификаторКандидата Из Кандидаты Цикл
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ИдентификаторПисьма", ИдентификаторКандидата);
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
...
// Забирать ли почту этой учётной записи по IMAP.
//
// POP3 у Gmail отдаёт только письма, ещё не выданные по этому протоколу: если
// в тот же ящик ходит Outlook, он забирает письма первым, и для 1С их больше
// не существует. IMAP такого понятия не знает — показывает папку целиком.
//
// Признак — заполненный порт IMAP в учётной записи: он проставлен там, где
// IMAP настроен. Не заполнен — учётная запись работает по POP3, как раньше.
//
Функция ПолучатьПоIMAP(УчетнаяЗапись) Экспорт

	Возврат УчетнаяЗапись.ПортIMAP_СД > 0
		И Не ПустаяСтрока(АдресСервераIMAP(УчетнаяЗапись));

КонецФункции

// Адрес IMAP-сервера. Отдельного поля в учётной записи нет, поэтому берём
// из адреса POP3: у почтовых служб они отличаются только приставкой
// (pop.gmail.com -> imap.gmail.com).
//
Функция АдресСервераIMAP(УчетнаяЗапись) Экспорт

	Адрес = НРег(СокрЛП(УчетнаяЗапись.POP3Сервер));

	Если Лев(Адрес, 5) = "pop3." Тогда
		Возврат "imap." + Сред(Адрес, 6);
	КонецЕсли;

	Если Лев(Адрес, 4) = "pop." Тогда
		Возврат "imap." + Сред(Адрес, 5);
	КонецЕсли;
...
// Подключение к ящику: по IMAP для перечисленных ящиков, иначе по POP3.
//
// Профиль передаётся, если он уже собран вызывающим кодом, — чтобы не собирать
// его дважды.
//
Процедура ПодключитьсяКЯщику(ИнтернетПочта, УчетнаяЗапись, Профиль = Неопределено) Экспорт

	Если Профиль = Неопределено Тогда
		Профиль = ПолучитьИнтернетПочтовыйПрофиль(УчетнаяЗапись);
	КонецЕсли;

	Если ПолучатьПоIMAP(УчетнаяЗапись) Тогда
		Профиль.АдресСервераIMAP = АдресСервераIMAP(УчетнаяЗапись);
		ИнтернетПочта.Подключиться(Профиль, ПротоколИнтернетПочты.IMAP);
		// Пишем протокол в журнал: по количеству писем IMAP и POP3 со стороны
		// не различить, а разница в поведении принципиальная.
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Информация, , ,
			Строка(УчетнаяЗапись) + ": підключення IMAP " + Профиль.АдресСервераIMAP
			+ ":" + УчетнаяЗапись.ПортIMAP_СД + ".");
	Иначе
		ИнтернетПочта.Подключиться(Профиль);
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Информация, , ,
			Строка(УчетнаяЗапись) + ": підключення POP3 " + УчетнаяЗапись.POP3Сервер
			+ ":" + УчетнаяЗапись.ПортPOP3 + ".");
	КонецЕсли;

КонецПроцедуры

// Заменяет кириллические буквы на латинские двойники, одинаковые по начертанию.
//
Функция ЗаменитьКириллическиеДвойники(Знач Текст) Экспорт

	Кириллица = "асеорхуАВСЕНКМОРТХіІ";
	Латиница  = "aceopxyABCEHKMOPTXiI";

	Результат = Текст;
	Для Сч = 1 По СтрДлина(Кириллица) Цикл
		Результат = СтрЗаменить(Результат, Сред(Кириллица, Сч, 1), Сред(Латиница, Сч, 1));
	КонецЦикла;
...
// Состоит ли адрес только из символов, допустимых в обычном (не IDN) адресе.
//
Функция ЭтоЛатинскийАдрес(Знач Адрес) Экспорт

	Допустимые = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.@_-+";

	Для Сч = 1 По СтрДлина(Адрес) Цикл
		Если Найти(Допустимые, Сред(Адрес, Сч, 1)) = 0 Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
// Приводит адрес к виду, который примет SMTP-сервер.
//
// Адреса попадают в письмо из контактной информации и руками, копированием из
// Word/Excel — вместе с ними приезжают знак абзаца ¶, табуляции, неразрывные
// пробелы, кавычки и угловые скобки. Gmail на такой RCPT TO отвечает
// «555 5.5.2 Syntax error» и не отправляет письмо вовсе. Чистим один раз здесь,
// перед отправкой, — тогда неважно, откуда пришёл мусор.
//
Функция НормализоватьАдресЭлектроннойПочты(Знач Адрес) Экспорт

	Мусор = Новый Массив;
	Мусор.Добавить(Символ(182));   // ¶
	Мусор.Добавить(Символ(160));   // неразрывный пробел
	Мусор.Добавить(Символ(8203));  // пробел нулевой ширины
	Мусор.Добавить(Символы.Таб);
	Мусор.Добавить(Символы.ПС);
	Мусор.Добавить(Символы.ВК);
	Мусор.Добавить(" ");
	Мусор.Добавить("""");
	Мусор.Добавить("'");
	Мусор.Добавить("<");
	...
	Результат = Строка(Адрес);
	Для Каждого Лишний Из Мусор Цикл
		Результат = СтрЗаменить(Результат, Лишний, "");
	КонецЦикла;

	// Кириллические буквы, неотличимые от латинских: адрес вида «сrocoash@gmail.com»
	// с кириллической «с» Gmail отвергает как синтаксическую ошибку (555 5.5.2).
	// Заменяем только тогда, когда после замены адрес становится полностью
	// латинским, — настоящие кириллические адреса (IDN) не трогаем.
	Латинский = ЗаменитьКириллическиеДвойники(Результат);
	Если Латинский <> Результат И ЭтоЛатинскийАдрес(Латинский) Тогда
		Результат = Латинский;
	КонецЕсли;

	// Хвостовые знаки препинания от копирования из текста: «адрес.» или «адрес;».
	Пока СтрДлина(Результат) > 0 И Найти(".,;:", Прав(Результат, 1)) > 0 Цикл
...
// Скачивает письма, не помечая их прочитанными на сервере.
//
// По IMAP метод Выбрать без третьего параметра ставит письмам на сервере флаг
// «прочитано». После перевода почты на IMAP из-за этого новые письма в Gmail
// и Outlook становились прочитанными сами, как только 1С их забирала, — ещё до
// того, как менеджер их открыл. Третий параметр (МаркироватьСообщения = Ложь)
// это отключает; на POP3 он ни на что не влияет.
//
// Если платформа третьего параметра не знает, падает ошибка о числе параметров —
// тогда вызываем по-старому, чтобы получение почты не сломалось. Любую другую
// ошибку (сбой письма, обрыв сессии) пробрасываем как есть: её обрабатывает
// вызывающий код.
//
Функция ВыбратьПисьмаНеПомечаяПрочитанными(ИнтернетПочта, УдалятьСообщения, Сообщения) Экспорт

	Попытка
		Возврат ИнтернетПочта.Выбрать(УдалятьСообщения, Сообщения, Ложь);
	Исключение
		ТекстОшибки = НРег(ОписаниеОшибки());
		Если Найти(ТекстОшибки, "параметр") = 0 Тогда
			ВызватьИсключение;
		КонецЕсли;
	КонецПопытки;

	ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Предупреждение, , ,
		"Платформа не поддерживает получение писем без пометки прочитанными — письма на сервере будут помечены прочитанными.");

	Возврат ИнтернетПочта.Выбрать(УдалятьСообщения, Сообщения);

КонецФункции

// Забирает в базу письма, отправленные мимо 1С: из Outlook, Gmail в браузере,
// с телефона. Они лежат в папке отправленных ящика, и без них переписка в
// структуре подчинённости рвётся: видно ответ поставщика, но не письмо
// менеджера, на которое он отвечал.
//
// Только IMAP — POP3 папок не знает.
//
// Дубли:
//  * письмо, уже лежащее в базе с любым статусом, узнаётся по ИдентификаторПисьма;
//  * письмо, отправленное из самой 1С, Gmail кладёт в отправленные с другим
//    идентификатором. Такое узнаём по теме, получателю и времени, заново не
//    создаём, а переписываем ему идентификатор на гугловский — тогда ответы на
//    него находят основание по заголовку In-Reply-To.
//
Процедура ПолучитьОтправленныеИзДругихПрограмм(ИнтернетПочта, УчетнаяЗапись, мРазделительАдресов) Экспорт

	// Доочистка писем, загруженных отсюда до исправления нумерации: порция за
	// заход, пока не кончатся. Стоит первой строкой — ниже есть досрочные выходы.
	ПронумероватьПисьмаБезНомера();
	ПривязатьПисьмаБезОснования();

	Папка = НайтиПапкуОтправленных(ИнтернетПочта, УчетнаяЗапись);
	Если Папка = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Обработанные = Новый Соответствие;   // ИдентификаторСообщения -> Истина
	ВсегоВПапке  = 0;
	Создано      = 0;
	Связано      = 0;
	Сбоев        = 0;
	...
	// Своя граница времени: вызывается в том же заходе после приёма входящих.
	КрайнийСрок  = ТекущаяДата() + 5 * 60;

	Пока Истина Цикл

		Если Не ВыбратьПапкуIMAP(ИнтернетПочта, УчетнаяЗапись, Папка) Тогда
			Прервать;
		КонецЕсли;
		...
		Исключение
			ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
				Строка(УчетнаяЗапись) + ": надіслані — не вдалося прочитати заголовки папки " + Папка + ". " + ОписаниеОшибки());
			Прервать;
		КонецПопытки;

		ВсегоВПапке = Заголовки.Количество();
		Заголовки   = ОтсечьСтарыеПисьма(Заголовки, УчетнаяЗапись);
		Заголовки   = УбратьУжеЗагруженныеПисьма(Заголовки, УчетнаяЗапись);

		НужноПереподключиться = Ложь;

		// С конца к началу — сначала свежие письма.
		Позиция = Заголовки.Количество() - 1;
		Пока Позиция >= 0 Цикл

			Заголовок = Заголовки[Позиция];
			Позиция   = Позиция - 1;

			Если ТекущаяДата() > КрайнийСрок Тогда
				Прервать;
			КонецЕсли;

			Идентификатор = "";
			Попытка
				Идентификатор = СокрЛП(Заголовок.ИдентификаторСообщения);
			Исключение
			КонецПопытки;

			Если Обработанные[Идентификатор] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Обработанные.Вставить(Идентификатор, Истина);

			// Отправлено из самой 1С — второй раз не создаём.
			Существующее = НайтиОтправленноеИз1С(Заголовок, УчетнаяЗапись);
			Если ЗначениеЗаполнено(Существующее) Тогда
				Если ПереписатьИдентификаторПисьма(Существующее, Идентификатор) Тогда
					Связано = Связано + 1;
				КонецЕсли;
				Продолжить;
			КонецЕсли;
			...
			Один = Новый Массив;
			Один.Добавить(Заголовок);

			Попытка
				Порция = ВыбратьПисьмаНеПомечаяПрочитанными(ИнтернетПочта, Ложь, Один);
			Исключение
				Сбоев       = Сбоев + 1;
				СбоевПодряд = СбоевПодряд + 1;
				ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
					Строка(УчетнаяЗапись) + ": надісланий лист пропущено. " + ОписаниеПисьма(Заголовок)
					+ " Помилка: " + ОписаниеОшибки());
				// Сбой рвёт сессию IMAP — выходим на переподключение.
				НужноПереподключиться = Истина;
				Прервать;
			КонецПопытки;

			Для Каждого ИнтернетПисьмо Из Порция Цикл
				Если ЗаписатьОтправленноеПисьмо(ИнтернетПисьмо, УчетнаяЗапись, мРазделительАдресов) Тогда
					Создано = Создано + 1;
				КонецЕсли;
			КонецЦикла;
			СбоевПодряд = 0;
		...
		Если Не НужноПереподключиться
			Или Сбоев >= 20
			Или СбоевПодряд >= 3
			Или ТекущаяДата() > КрайнийСрок Тогда
			Прервать;
		КонецЕсли;

		// Переподключение сбрасывает текущую папку — в начале цикла встаём в неё снова.
		Если Не ПереподключитьПочту(ИнтернетПочта, УчетнаяЗапись) Тогда
			Прервать;
		КонецЕсли;
	...
	ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Информация, , ,
		Строка(УчетнаяЗапись) + ": надіслані («" + Папка + "») — у папці " + ВсегоВПапке
		+ ", записано нових " + Создано + ", зв'язано з відправленими з 1С " + Связано
		+ ", пропущено " + Сбоев + ".");

КонецПроцедуры

// Имя папки отправленных в ящике или Неопределено.
//
// Если папка не нашлась, в журнал пишется список папок ящика — по нему видно,
// как сервер её называет, и имя можно добавить в ИменаПапкиОтправленных.
//
Функция НайтиПапкуОтправленных(ИнтернетПочта, УчетнаяЗапись) Экспорт

	Попытка
		Ящики = ИнтернетПочта.ПолучитьПочтовыеЯщики();
	Исключение
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Предупреждение, , ,
			Строка(УчетнаяЗапись) + ": надіслані — список папок IMAP не отримано. " + ОписаниеОшибки());
		Возврат Неопределено;
	КонецПопытки;

	Имена  = ИменаПапкиОтправленных();
	Список = "";

	Для Каждого Ящик Из Ящики Цикл

		ИмяЯщика = Строка(Ящик);
		Список   = Список + ?(ПустаяСтрока(Список), "", "; ") + ИмяЯщика;

		Для Каждого Имя Из Имена Цикл
			Если НРег(ИмяЯщика) = НРег(Имя)
				Или НРег(Прав(ИмяЯщика, СтрДлина(Имя) + 1)) = НРег("/" + Имя) Тогда
				Возврат ИмяЯщика;
			КонецЕсли;
		КонецЦикла;
	...
	ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Предупреждение, , ,
		Строка(УчетнаяЗапись) + ": надіслані — папку не знайдено. Папки скриньки: " + Лев(Список, 1000));

	Возврат Неопределено;

КонецФункции

// Как называется папка отправленных в разных почтовых службах и языках интерфейса.
// Кириллические имена по IMAP приходят в кодировке modified UTF-7, поэтому
// перечислены в обоих видах: «Надіслані» = &BB0EMAQ0BFYEQQQ7BDAEPQRW-.
//
Функция ИменаПапкиОтправленных() Экспорт

	Имена = Новый Массив;
	Имена.Добавить("Sent Mail");
	Имена.Добавить("Sent Items");
	Имена.Добавить("Sent");
	Имена.Добавить("Надіслані");
	Имена.Добавить("&BB0EMAQ0BFYEQQQ7BDAEPQRW-");
	Имена.Добавить("Відправлені");
	Имена.Добавить("&BBIEVgQ0BD8EQAQwBDIEOwQ1BD0EVg-");
	Имена.Добавить("Отправленные");
	Имена.Добавить("&BB4EQgQ,BEAEMAQyBDsENQQ9BD0ESwQ1-");

	Возврат Имена;

КонецФункции

// Встаёт в папку IMAP. Ложь — если сервер или платформа не дали этого сделать.
//
Функция ВыбратьПапкуIMAP(ИнтернетПочта, УчетнаяЗапись, Папка) Экспорт

	Попытка
		ИнтернетПочта.ТекущийПочтовыйЯщик = Папка;
		Возврат Истина;
	Исключение
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
			Строка(УчетнаяЗапись) + ": надіслані — не вдалося відкрити папку " + Папка + ". " + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;

КонецФункции

// Убирает из списка письма, которые уже есть в базе у этой учётной записи с
// любым статусом. Типовая проверка смотрит только полученные и для папки
// отправленных не годится: письма скачивались бы заново каждый заход.
//
Функция УбратьУжеЗагруженныеПисьма(Заголовки, УчетнаяЗапись) Экспорт

	Идентификаторы = Новый Массив;
	Для Каждого Заголовок Из Заголовки Цикл
		Попытка
			Идентификаторы.Добавить(Лев(СокрЛП(Заголовок.ИдентификаторСообщения), 150));
		Исключение
		КонецПопытки;
	КонецЦикла;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЭлектронноеПисьмо.ИдентификаторПисьма КАК Идентификатор
	|ИЗ
	|	Документ.ЭлектронноеПисьмо КАК ЭлектронноеПисьмо
	|ГДЕ
	|	ЭлектронноеПисьмо.УчетнаяЗапись = &УчетнаяЗапись
	|	И ЭлектронноеПисьмо.ИдентификаторПисьма В (&Идентификаторы)";
	...
	Для Каждого Заголовок Из Заголовки Цикл
		Идентификатор = "";
		Попытка
			Идентификатор = Лев(СокрЛП(Заголовок.ИдентификаторСообщения), 150);
		Исключение
		КонецПопытки;
		Если Загруженные[Идентификатор] = Неопределено Тогда
			Новые.Добавить(Заголовок);
		КонецЕсли;
	КонецЦикла;
...
// Ищет письмо, отправленное из 1С, копию которого Gmail положил в отправленные.
//
// Сверяются тема, первый получатель и время отправки ±15 минут. Одной темы мало:
// письма поставщикам по одному предзаказу уходят с одинаковой темой, отличает их
// только адрес. Адрес в табличной части хранится усечённым до 50 символов —
// сравниваем с усечённым.
//
Функция НайтиОтправленноеИз1С(Заголовок, УчетнаяЗапись) Экспорт

	Попытка
		ТемаПисьма = Лев(Заголовок.Тема, 400);
		ДатаПисьма = Заголовок.ДатаОтправления;
	Исключение
		Возврат Неопределено;
	КонецПопытки;

	Если ПустаяСтрока(ТемаПисьма) Или Не ЗначениеЗаполнено(ДатаПисьма) Тогда
		Возврат Неопределено;
	КонецЕсли;

	Адрес = "";
	Попытка
		Для Каждого Получатель Из Заголовок.Получатели Цикл
			Адрес = Лев(НормализоватьАдресЭлектроннойПочты(Получатель.Адрес), 50);
			Прервать;
		КонецЦикла;
	Исключение
	КонецПопытки;
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("УчетнаяЗапись", УчетнаяЗапись);
	Запрос.УстановитьПараметр("Тема"         , ТемаПисьма);
	Запрос.УстановитьПараметр("Адрес"        , Адрес);
	Запрос.УстановитьПараметр("С"            , ДатаПисьма - 15 * 60);
	Запрос.УстановитьПараметр("По"           , ДатаПисьма + 15 * 60);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	КомуТЧ.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭлектронноеПисьмо.КомуТЧ КАК КомуТЧ
	|ГДЕ
	|	КомуТЧ.Ссылка.УчетнаяЗапись = &УчетнаяЗапись
	|	И КомуТЧ.Ссылка.СтатусПисьма = ЗНАЧЕНИЕ(Перечисление.СтатусыПисем.Отправленное)
	|	И КомуТЧ.Ссылка.Тема = &Тема
	|	И КомуТЧ.Ссылка.ДатаОтправления МЕЖДУ &С И &По
	|	И КомуТЧ.АдресЭлектроннойПочты = &Адрес";
...
// Переписывает письму идентификатор на тот, под которым его знает получатель.
// Меняется только служебное поле, поэтому обработчики записи письма не нужны.
//
Функция ПереписатьИдентификаторПисьма(Ссылка, Знач Идентификатор) Экспорт

	Идентификатор = Лев(СокрЛП(Идентификатор), 150);
	Если ПустаяСтрока(Идентификатор) Или Ссылка.ИдентификаторПисьма = Идентификатор Тогда
		Возврат Ложь;
	КонецЕсли;

	Попытка
		Объект = Ссылка.ПолучитьОбъект();
		Объект.ИдентификаторПисьма = Идентификатор;
		Объект.ОбменДанными.Загрузка = Истина;
		Объект.Записать();
		Возврат Истина;
	Исключение
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , Ссылка,
			"Не вдалося переписати ідентифікатор листа: " + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;

КонецФункции

// Доносит основание письмам, которые остались без него из-за прежнего разбора
// темы (буквы RE внутри слова ломали снятие префикса).
//
// Берётся порция свежих писем без основания, по каждому вызывается тот же
// типовой поиск, что и при получении. Письмо записывается в режиме загрузки:
// меняется только реквизит основания.
//
Процедура ПривязатьПисьмаБезОснования(Знач Порция = 100, Знач ДнейНазад = 30) Экспорт

	Попытка

		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Граница", НачалоДня(ТекущаяДата() - ДнейНазад * 24 * 60 * 60));
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ " + Формат(Порция, "ЧГ=0") + "
		|	ЭлектронноеПисьмо.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.ЭлектронноеПисьмо КАК ЭлектронноеПисьмо
		|ГДЕ
		|	ЭлектронноеПисьмо.Дата >= &Граница
		|	И ЭлектронноеПисьмо.ОснованиеПисьма = НЕОПРЕДЕЛЕНО
		|	И НЕ ЭлектронноеПисьмо.ПометкаУдаления
		|	И ЭлектронноеПисьмо.СтатусПисьма = ЗНАЧЕНИЕ(Перечисление.СтатусыПисем.Полученное)
		|	И ЭлектронноеПисьмо.УчетнаяЗапись.ДляВходящихОтветовИПереадресацийИскатьПисьмаОснованияИЗаполнятьОснованиеНовогоПисьма
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Исключение
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
			"Прив'язка листів без підстави: не вдалося вибрати листи. " + ОписаниеОшибки());
		Возврат;
	КонецПопытки;
	...
	Пока Выборка.Следующий() Цикл
		Попытка
			...
			Если ЗначениеЗаполнено(Объект.ОснованиеПисьма) Или ПустаяСтрока(Объект.Тема) Тогда
				Продолжить;
			КонецЕсли;

			НайтиИПроставитьОснованиеПисьма(Объект);

			Если ЗначениеЗаполнено(Объект.ОснованиеПисьма) Тогда
				Объект.ОбменДанными.Загрузка = Истина;
				Объект.Записать();
				Привязано = Привязано + 1;
			КонецЕсли;

		Исключение
			ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , Выборка.Ссылка,
				"Прив'язка листа без підстави не вдалася: " + ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;

	Если Привязано > 0 Тогда
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Информация, , ,
			"Прив'язка листів без підстави: прив'язано " + Привязано + ".");
	КонецЕсли;

КонецПроцедуры

// Назначает номера письмам, записанным без номера.
//
// Письма из папки отправленных до исправления писались в режиме загрузки, и
// платформа номер не ставила — таких набралось больше полутора тысяч. Пустой
// номер мешает искать письмо и выглядит в структуре подчинённости как сбой.
// Номера раздаются в порядке дат писем, порцией за раз, чтобы не растягивать
// заход получения почты. Ошибка одного письма не останавливает остальные.
//
Процедура ПронумероватьПисьмаБезНомера(Знач Порция = 200) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
			"Нумерація листів без номера: не вдалося вибрати листи. " + ОписаниеОшибки());
		Возврат;
	КонецПопытки;
	...
	Пока Выборка.Следующий() Цикл
		Попытка
			...
			Если ПустаяСтрока(Объект.Номер) Тогда
				...
				// Меняется только номер: обработчики записи письма здесь не нужны,
				// тем же режимом письмо и было записано.
				Объект.ОбменДанными.Загрузка = Истина;
				Объект.Записать();
				Пронумеровано = Пронумеровано + 1;
			КонецЕсли;
		Исключение
			Ошибок = Ошибок + 1;
			ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , Выборка.Ссылка,
				"Нумерація листа без номера не вдалася: " + ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;

	Если Пронумеровано > 0 Или Ошибок > 0 Тогда
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Информация, , ,
			"Нумерація листів без номера: пронумеровано " + Пронумеровано + ", помилок " + Ошибок + ".");
	КонецЕсли;

КонецПроцедуры

// Записывает в базу копию письма из папки отправленных — со статусом
// «Отправленное» и в группу «Исходящие», вместе с вложениями.
//
// Пишется с ОбменДанными.Загрузка: обработчики записи письма рассчитаны на письмо,
// которое создают в 1С, — подменяют отправителя данными учётной записи, ставят
// основанию пометку «ответ отправлен», берут ответственного из клиентской
// глобальной переменной (на сервере регламента её нет). Для копии уже
// отправленного письма всё это не нужно, нужные поля проставляются здесь.
//
Функция ЗаписатьОтправленноеПисьмо(ИнтернетПисьмо, УчетнаяЗапись, мРазделительАдресов) Экспорт
	...
	Попытка
		...
		Письмо.НеРассмотрено       = Ложь;
		Письмо.Ответственный       = УчетнаяЗапись.ОтветственныйЗаАвтоПолучениеОтправкуСообщений;

		// Дата документа — время отправки, а не время загрузки: по ней дерево
		// переписки сортируется в структуре подчинённости.
		Если ЗначениеЗаполнено(Письмо.ДатаОтправления) Тогда
			Письмо.Дата           = Письмо.ДатаОтправления;
			Письмо.ДатаТранспорта = Письмо.ДатаОтправления;
		КонецЕсли;
		...
		// В режиме загрузки платформа номер не назначает — без этого письма из
		// «Отправленных» ложились в базу с пустым номером. Префикс ставит подписка
		// ПриУстановкеНовогоНомераДокумента, режим загрузки ей не мешает.
		Если ПустаяСтрока(Письмо.Номер) Тогда
			Письмо.УстановитьНовыйНомер();
		КонецЕсли;
		Письмо.ОбменДанными.Загрузка = Истина;
		Письмо.Записать();
	...
	Исключение
		...
		КонецЕсли;
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
			Строка(УчетнаяЗапись) + ": надісланий лист не записано. " + ОписаниеПисьма(ИнтернетПисьмо)
			+ " Помилка: " + ТекстОшибки);
		Возврат Ложь;
...
// Идентификаторы писем из заголовка In-Reply-To или References.
//
// Значение заголовка — один или несколько идентификаторов в угловых скобках,
// References ещё и переносится на несколько строк. Возвращаются без скобок,
// в том же виде, в каком лежат в реквизите ИдентификаторПисьма.
//
Функция ИдентификаторыИзЗаголовкаПисьма(ИнтернетПисьмо, ИмяЗаголовка) Экспорт

	Идентификаторы = Новый Массив;

	Значение = "";
	Попытка
		Значение = ИнтернетПисьмо.ПолучитьПолеЗаголовка(ИмяЗаголовка, "Строка");
	Исключение
	...
	Остаток = Значение;
	Начало  = Найти(Остаток, "<");
	Пока Начало > 0 Цикл
		Остаток = Сред(Остаток, Начало + 1);
		Конец   = Найти(Остаток, ">");
		Если Конец = 0 Тогда
			Прервать;
		КонецЕсли;
		Идентификатор = СокрЛП(Лев(Остаток, Конец - 1));
		Если Не ПустаяСтрока(Идентификатор) И Идентификаторы.Найти(Идентификатор) = Неопределено Тогда
			Идентификаторы.Добавить(Идентификатор);
		КонецЕсли;
		Остаток = Сред(Остаток, Конец + 1);
		Начало  = Найти(Остаток, "<");
	КонецЦикла;

	// Бывает и без угловых скобок — тогда значение целиком.
	Если Идентификаторы.Количество() = 0 И Найти(Значение, " ") = 0 Тогда
		Идентификаторы.Добавить(СокрЛП(Значение));
	КонецЕсли;
...
// Дата, начиная с которой письма забираются в базу.
//
// В ящиках лежит архив за годы: Gmail по POP3 отдаёт очередь нескачанных писем
// с самого старого, и без отсечки база наполняется перепиской 2022 года,
// а свежая почта ждёт своей очереди.
//
// Менять здесь — одно место на всю почту.
//
Функция ДатаНачалаПолученияПисем() Экспорт

	Возврат '20260101';

КонецФункции

// Убирает из списка письма старше даты начала получения.
//
// Письмо без даты отправления не отсекаем: дата — то, что пишет отправитель,
// и у части писем её просто нет; лучше лишнее письмо, чем потерянное.
//
// Параметры
//  МассивЗаголовков - Массив ИнтернетПочтовоеСообщение
//  УчетнаяЗапись    - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты, для журнала
//
// Возвращаемое значение:
//  Массив - только письма от даты начала получения
//
Функция ОтсечьСтарыеПисьма(МассивЗаголовков, УчетнаяЗапись) Экспорт

	Граница = ДатаНачалаПолученияПисем();
	Свежие  = Новый Массив;
	Старых  = 0;
	...
	Для Каждого Заголовок Из МассивЗаголовков Цикл
		...
		Дата = '00010101';
		Попытка
			Дата = Заголовок.ДатаОтправления;
		Исключение
		КонецПопытки;

		Если ЗначениеЗаполнено(Дата) И Дата < Граница Тогда
			Старых = Старых + 1;
			Продолжить;
		КонецЕсли;

		Свежие.Добавить(Заголовок);

	КонецЦикла;

	Если Старых > 0 Тогда
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Информация, , ,
			Строка(УчетнаяЗапись) + ": пропущено " + Старых + " листів старіше "
			+ Формат(Граница, "ДЛФ=D") + ", до отримання " + Свежие.Количество() + ".");
	КонецЕсли;
...
// Получает письма по одному, пропуская те, на которых платформа спотыкается.
//
// Вызывается после того, как выборка всей пачки упала. К этому моменту сессия
// POP3 уже мертва: сервер закрыл соединение, и в ней падает любое письмо, даже
// на 500 байт. Поэтому первым делом переподключаемся, а после каждого сбоя —
// заново: одно неразбираемое письмо рвёт соединение, и без переподключения все
// следующие письма помечались бы сбойными без всякой вины.
//
// Проблемное письмо описывается в журнале (событие «Почта.Получение») — по теме,
// отправителю, размеру и Message-ID его можно найти в ящике руками.
//
// Параметры
//  ИнтернетПочта    - ИнтернетПочта, соединение (будет переоткрыто)
//  УчетнаяЗапись    - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты
//  МассивЗаголовков - Массив, заголовки из упавшей попытки; служит только
//                     оценкой объёма, заголовки перечитываются после подключения
//
// Возвращаемое значение:
//  Массив - полученные письма
//
Функция ПолучитьПисьмаПоОдному(ИнтернетПочта, УчетнаяЗапись, МассивЗаголовков, МассивПисемДляУдаления) Экспорт

	Полученные   = Новый Массив;
	Обработанные = Новый Соответствие;   // ИдентификаторСообщения -> Истина
	Сбоев        = 0;

	// Ограничители, чтобы задание не висело часами на большом ящике и не
	// налезало на следующий запуск регламента: письма не считаем, считаем время.
	// Недобранное заберётся следующим заходом.
	МаксимумСбоев  = 20;
	// Если письма перестают читаться подряд, дело не в отдельном письме, а в
	// ящике или сервере: продолжать бессмысленно, только сессии на Gmail жжём.
	МаксимумСбоевПодряд = 3;
	СбоевПодряд    = 0;
	МаксимумМинут  = 8;
	КрайнийСрок    = ТекущаяДата() + МаксимумМинут * 60;
	...
	Пока Истина Цикл
		...
		Попытка
			Заголовки = ИнтернетПочта.ПолучитьЗаголовки();
			// Тем же фильтром, что и основной путь: уже полученные письма заново
			// не скачиваем, иначе заход уходит на разбор архива по второму кругу.
			Заголовки = ПолучитьСписокПисемКПолучению(УчетнаяЗапись, Заголовки, МассивПисемДляУдаления);
			Заголовки = ОтсечьСтарыеПисьма(Заголовки, УчетнаяЗапись);
		Исключение
			ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
				Строка(УчетнаяЗапись) + ": не вдалося перечитати заголовки. " + ОписаниеОшибки());
			Прервать;
		КонецПопытки;

		ОсталосьРабота = Ложь;

		// С конца к началу: POP3 отдаёт письма в порядке поступления, а нужны
		// сначала свежие — иначе заход целиком уходит на письма 2022 года.
		Номер = Заголовки.Количество() - 1;
		Пока Номер >= 0 Цикл

			Заголовок = Заголовки[Номер];
			Номер = Номер - 1;
			...
			Попытка
				...
				Порция = ВыбратьПисьмаНеПомечаяПрочитанными(ИнтернетПочта, НЕ УчетнаяЗапись.ОставлятьКопииСообщенийНаСервере, Один);
				Для Каждого Письмо Из Порция Цикл
					Полученные.Добавить(Письмо);
				КонецЦикла;
				Обработанные.Вставить(Идентификатор, Истина);
				СбоевПодряд = 0;
			...
			Исключение
				...
				Обработанные.Вставить(Идентификатор, Истина);
				ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
					Строка(УчетнаяЗапись) + ": лист пропущено. " + ОписаниеПисьма(Заголовок)
					+ " Помилка: " + ОписаниеОшибки());

				// Сбой рвёт сессию — выходим на переподключение, иначе следующие
				// письма упадут просто потому, что соединения больше нет.
				Прервать;
		...
		Если СбоевПодряд >= МаксимумСбоевПодряд Тогда
			ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Предупреждение, , ,
				Строка(УчетнаяЗапись) + ": " + СбоевПодряд + " листи поспіль не читаються — захід зупинено.");
			Прервать;
		КонецЕсли;

		Если Не ОсталосьРабота
			Или Сбоев >= МаксимумСбоев
			Или ТекущаяДата() > КрайнийСрок Тогда
			Прервать;
		КонецЕсли;
	...
	ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Информация, , ,
		Строка(УчетнаяЗапись) + ": отримано по одному " + Полученные.Количество()
		+ ", пропущено " + Сбоев + ", у скриньці було " + МассивЗаголовков.Количество() + ".");

	Возврат Полученные;

КонецФункции

// Переоткрывает соединение с почтовым сервером.
//
// Возвращаемое значение:
//  Булево - удалось ли подключиться
//
Функция ПереподключитьПочту(ИнтернетПочта, УчетнаяЗапись) Экспорт

	Попытка
		ИнтернетПочта.Отключиться();
	Исключение
		// Соединения уже нет — это и есть обычный случай здесь.
	КонецПопытки;

	Попытка
		ПодключитьсяКЯщику(ИнтернетПочта, УчетнаяЗапись);
		Возврат Истина;
	Исключение
		ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
			Строка(УчетнаяЗапись) + ": не вдалося перепідключитися. " + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;

КонецФункции

// Короткое описание письма для журнала: по нему письмо ищут в ящике руками.
// Каждое обращение к заголовку в своей попытке — у сбойного письма может
// не читаться и сама тема.
//
Функция ОписаниеПисьма(Заголовок) Экспорт

	Описание = "";

	Попытка
		Описание = Описание + " Тема: " + Лев(СокрЛП(Заголовок.Тема), 100) + ".";
	Исключение
	КонецПопытки;

	Попытка
		Описание = Описание + " Від: " + СокрЛП(Строка(Заголовок.Отправитель.Адрес)) + ".";
	Исключение
	КонецПопытки;

	Попытка
		Описание = Описание + " Розмір: " + Заголовок.Размер + " байт.";
	Исключение
	КонецПопытки;

	Попытка
		Описание = Описание + " ID: " + СокрЛП(Заголовок.ИдентификаторСообщения) + ".";
	Исключение
	КонецПопытки;
...
Процедура ПолучениеОтправкаПисем(СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, МассивУчетныхЗаписей = Неопределено, 
	...
	// ОТЛАДКА ПОЧТЫ
	ОтладкаПочтыВЖурнал("Старт: пользователь = " + Строка(ТекущийПользователь)
		+ ", отправить = " + Строка(Отправить) + ", получить = " + Строка(Получить)
		+ ", учётных записей с правом записи = " + Строка(СписокДоступныхУчетныхЗаписей.Количество())
		+ " (" + ПредставлениеСпискаУчетныхЗаписей(СписокДоступныхУчетныхЗаписей) + ")");
	...
	Если Отправить Тогда
		...
		// ОТЛАДКА ПОЧТЫ
		ДляОтладки = "";
		Для каждого СтрокаДереваОтладки Из ДеревоУчетныхЗаписейИПисем.Строки Цикл
			ДляОтладки = ДляОтладки + ?(ПустаяСтрока(ДляОтладки), "", "; ")
				+ Строка(СтрокаДереваОтладки.УчетнаяЗапись) + " - писем: " + Строка(СтрокаДереваОтладки.Строки.Количество());
		КонецЦикла;
		ОтладкаПочтыВЖурнал("Отобрано к отправке: " + ?(ПустаяСтрока(ДляОтладки), "ничего", ДляОтладки));
	...
	Если СоответствиеУчетныхЗаписейВсего.Количество() > 0 Тогда
		...
		Для каждого ЭлементСоответствия Из СоответствиеУчетныхЗаписейВсего Цикл
			...
			// ОТЛАДКА ПОЧТЫ
			ОтладкаПочтыВЖурнал("Подключение: " + ПредставлениеПрофиляДляОтладки(УчетнаяЗапись, Профиль));
			
			Попытка
				ПодключитьсяКЯщику(ИнтернетПочта, УчетнаяЗапись, Профиль);
				// ОТЛАДКА ПОЧТЫ
				ОтладкаПочтыВЖурнал("Подключение УСПЕШНО: " + Строка(УчетнаяЗапись));
			Исключение
				
				// ОТЛАДКА ПОЧТЫ
				ОтладкаПочтыВЖурнал("Подключение ОШИБКА: " + ПредставлениеПрофиляДляОтладки(УчетнаяЗапись, Профиль)
				...
				Если ТипЗнч(ДеревоУчетныхЗаписейИПисем) = Тип("ДеревоЗначений") Тогда
					...
					Если СтрокаДереваУчетнойЗаписи <> Неопределено Тогда
						...
						Для каждого СтрокаПисьма Из СтрокаДереваУчетнойЗаписи.Строки Цикл
							...
							// Цепочка переписки: In-Reply-To / References по письму-основанию.
							УстановитьЗаголовкиЦепочки(ПочтовоеСообщение, Письмо);
							
							Для каждого СтрокаТЧ Из Письмо.КомуТЧ Цикл
								Получатель = ПочтовоеСообщение.Получатели.Добавить();
								Получатель.Адрес           = НормализоватьАдресЭлектроннойПочты(СтрокаТЧ.АдресЭлектроннойПочты);
							...
							Для каждого СтрокаТЧ Из Письмо.КопииТЧ Цикл
								...
								Копия.Адрес           = НормализоватьАдресЭлектроннойПочты(СтрокаТЧ.АдресЭлектроннойПочты);
							...
							Для каждого СтрокаТЧ Из Письмо.СкрытыеКопииТЧ Цикл
								СлепаяКопия = ПочтовоеСообщение.СлепыеКопии.Добавить(НормализоватьАдресЭлектроннойПочты(СтрокаТЧ.АдресЭлектроннойПочты));
								СлепаяКопия.Адрес           = НормализоватьАдресЭлектроннойПочты(СтрокаТЧ.АдресЭлектроннойПочты);
							...
							Попытка
								...
								// ОТЛАДКА ПОЧТЫ
								ОтладкаПочтыВЖурнал("Письмо ОТПРАВЛЕНО: " + Строка(УчетнаяЗапись)
									+ ", письмо = " + Строка(Письмо) + ", тема = " + Строка(ПочтовоеСообщение.Тема));
							...
							Исключение
								...
								// ОТЛАДКА ПОЧТЫ
								ОтладкаПочтыВЖурнал("Письмо НЕ ОТПРАВЛЕНО: " + Строка(УчетнаяЗапись)
									+ ", письмо = " + Строка(Письмо) + ". "
									+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Истина);
			...
			Если Получить Тогда
				...
				МассивЗаголовковПисем = ОтсечьСтарыеПисьма(МассивЗаголовковПисем, УчетнаяЗапись);
				Если МассивЗаголовковПисем.Количество() > 0 Тогда
				
					// Одно письмо, которое платформа не может разобрать (большое вложение,
					// битый MIME), роняло весь заход: письма запрашиваются пачкой, и почта
					// не забиралась вовсе. Поэтому при сбое пачки переходим на выборку
					// по одному — сбойное пропускаем и называем в журнале.
					Попытка
						МассивПисем = ВыбратьПисьмаНеПомечаяПрочитанными(ИнтернетПочта, НЕ УчетнаяЗапись.ОставлятьКопииСообщенийНаСервере, МассивЗаголовковПисем);
					Исключение
						ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Предупреждение, , ,
							Строка(УчетнаяЗапись) + ": пачку писем получить не удалось (" + ОписаниеОшибки()
							+ "). Перехожу на получение по одному письму.");
									МассивПисем = ПолучитьПисьмаПоОдному(ИнтернетПочта, УчетнаяЗапись, МассивЗаголовковПисем, МассивПисемДляУдаления);
					КонецПопытки;
			...
			// Отправленные из других программ — Outlook, Gmail в браузере, телефон. Такие
			// письма лежат в папке отправленных ящика, а приём выше читает только «Входящие».
			Если Получить И ПолучатьПоIMAP(УчетнаяЗапись) Тогда
				Попытка
					ПолучитьОтправленныеИзДругихПрограмм(ИнтернетПочта, УчетнаяЗапись, мРазделительАдресов);
				Исключение
					ЗаписьЖурналаРегистрации("Почта.Получение", УровеньЖурналаРегистрации.Ошибка, , ,
						Строка(УчетнаяЗапись) + ": надіслані не отримано. " + ОписаниеОшибки());
				КонецПопытки;
			КонецЕсли;
			
			// ОТЛАДКА ПОЧТЫ
			ОтладкаПочтыВЖурнал("Отключение: " + Строка(УчетнаяЗапись)
				+ ", накопленные ошибки: " + ?(ПустаяСтрока(ТекстОшибок), "нет", СокрЛП(ТекстОшибок)));
	...
	Если Письмо.УчетнаяЗапись.ДляВходящихОтветовИПереадресацийИскатьПисьмаОснованияИЗаполнятьПредмет
	 ...
	 ИЛИ Письмо.УчетнаяЗапись.ДляВходящихОтветовИПереадресацийИскатьПисьмаОснованияИЗаполнятьГруппуПисемНовогоПисьма Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			КонецЕсли; 
			// Основание, найденное по заголовкам цепочки, точнее поиска по теме: тема
			// ведёт к первому нашему письму, а заголовок — к письму, на которое
			// ответили на самом деле. Перезаписываем только пустое.
			Если Письмо.УчетнаяЗапись.ДляВходящихОтветовИПереадресацийИскатьПисьмаОснованияИЗаполнятьОснованиеНовогоПисьма
				И Не ЗначениеЗаполнено(Письмо.ОснованиеПисьма) Тогда
				Письмо.ОснованиеПисьма = Выборка.Ссылка;
			КонецЕсли; 
...
Функция ПреобразоватьТекстИзХТМЛФорматаВПростой(ТекстВФорматеХТМЛ) Экспорт
	...
	// Основной путь — разбор через COM-объект HtmlFile (движок Internet Explorer).
	// На части машин у документа нет ни tags, ни innerText, и тогда просмотр письма
	// падал целиком. Не вышло — снимаем разметку своими силами.
	Попытка
	...
	Исключение
		Возврат ТекстБезРазметкиHTML(ТекстВФорматеХТМЛ);
	КонецПопытки;

КонецФункции

// Запасной разбор HTML без COM: выбрасывает теги и раскрывает основные подстановки.
// Читается хуже, чем через движок браузера, но письмо остаётся читаемым,
// а форма не падает.
//
Функция ТекстБезРазметкиHTML(Знач Текст) Экспорт

	// Содержимое script и style в текст письма не входит.
	Текст = ВырезатьБлокHTML(Текст, "<script", "</script>");
	Текст = ВырезатьБлокHTML(Текст, "<style", "</style>");

	// Переводы строк там, где они были разметкой.
	Для Каждого Тег Из ТегиПереносовHTML() Цикл
		Текст = СтрЗаменить(Текст, Тег, Символы.ПС);
	КонецЦикла;
...
// Теги, на месте которых в простом тексте остаётся перевод строки.
//
Функция ТегиПереносовHTML()
...
// Вырезает блоки вида <script …> … </script> вместе с содержимым.
//
Функция ВырезатьБлокHTML(Знач Текст, Знач Начало, Знач Конец)

	ПозицияНачала = Найти(НРег(Текст), Начало);
	Защита = 0;

	Пока ПозицияНачала > 0 И Защита < 200 Цикл

		Защита = Защита + 1;
		Остаток = Сред(НРег(Текст), ПозицияНачала);
		ПозицияКонца = Найти(Остаток, Конец);

		Если ПозицияКонца = 0 Тогда
			Возврат Лев(Текст, ПозицияНачала - 1);
		КонецЕсли;

		Текст = Лев(Текст, ПозицияНачала - 1) + Сред(Текст, ПозицияНачала + ПозицияКонца + СтрДлина(Конец) - 1);
		ПозицияНачала = Найти(НРег(Текст), Начало);
...
Функция ИсключитьПрефиксыТемыПисьма(Знач Тема) Экспорт
	...
	// Префиксы снимаются только с начала темы и по одному, пока они там есть.
	// Прежний разбор искал «RE» подстрокой в любом месте: в теме «Re: Ionity.tires
	// 152405» он цеплялся за «re» внутри слова tires, уезжал за конец строки и
	// возвращал тему вместе с «Re:». Из-за этого ответ поставщика не находил
	// исходное письмо и оставался без основания.
	
	Результат = СокрЛП(Тема);
	
	Префиксы = Новый Массив;
	Префиксы.Добавить("FWD");
	Префиксы.Добавить("FW");
	Префиксы.Добавить("RE");
	
	Обрезали = Истина;
	Защита   = 0;
	
	Пока Обрезали И Защита < 20 Цикл
		...
		Для Каждого Префикс Из Префиксы Цикл
			...
			Если Врег(Лев(Результат, СтрДлина(Префикс))) <> Префикс Тогда
				Продолжить;
			КонецЕсли;
			
			Остаток = СокрЛ(Сред(Результат, СтрДлина(Префикс) + 1));
			
			// Счётчик повторов: Re[2]: или Re(2):
			Если Лев(Остаток, 1) = "[" Или Лев(Остаток, 1) = "(" Тогда
				Закрывающая = ?(Лев(Остаток, 1) = "[", "]", ")");
				ПозицияЗакрывающей = Найти(Остаток, Закрывающая);
				Если ПозицияЗакрывающей > 0 Тогда
					Остаток = СокрЛ(Сред(Остаток, ПозицияЗакрывающей + 1));
				КонецЕсли;
			КонецЕсли;
			
			Если Лев(Остаток, 1) = ":" Тогда
				Результат = СокрЛП(Сред(Остаток, 2));
				Обрезали  = Истина;
				Прервать;
			КонецЕсли;
...
Функция ПолучитьИнтернетПочтовыйПрофиль(УчетнаяЗапись) Экспорт
	...
	Профиль.ПортSMTP         = УчетнаяЗапись.ПортSMTP;
	Профиль.ПортPOP3         = УчетнаяЗапись.ПортPOP3;
	ПРофиль.ПортIMAP		 = УчетнаяЗапись.ПортIMAP_СД;//С.Д. Сеня 01,08,23 + было УчетнаяЗапись.ПортPOP3; //**
	...
	Если УчетнаяЗапись.ТребуетсяSMTPАутентификация Тогда
		...
		Профиль.ПарольIMAP         = УчетнаяЗапись.ПарольSMTP;
		Профиль.ПользовательIMAP   = УчетнаяЗапись.ЛогинSMTP;
	Иначе
		Профиль.АутентификацияSMTP = СпособSMTPАутентификации.БезАутентификации;
		Профиль.ПарольSMTP         = "";
		Профиль.ПользовательSMTP   = "";
...
//
// Временный блок: ошибки отправки типовой код копит в ТекстОшибок и показывает
// только на форме индикации, а при ПоказыватьОкноВыполнения = Ложь теряет молча.
// Пишем ход отправки в журнал регистрации, событие "Почта.Отладка".
// Читается снаружи через /db/cmd, действие log, event = "Почта.Отладка".

Процедура ОтладкаПочтыВЖурнал(Знач Текст, Знач ЭтоОшибка = Ложь) Экспорт

	Если Не ВключенаОтладкаПочты() Тогда
		Возврат;
	КонецЕсли;

	Попытка
		Уровень = ?(ЭтоОшибка, УровеньЖурналаРегистрации.Ошибка, УровеньЖурналаРегистрации.Информация);
		ЗаписьЖурналаРегистрации("Почта.Отладка", Уровень, , , Лев(Строка(Текст), 4000));
	Исключение
		// отладка не должна ронять отправку
	КонецПопытки;

КонецПроцедуры

// Пока разбираемся, почему часть учётных записей не отправляет, отладка включена всегда.
Функция ВключенаОтладкаПочты() Экспорт

	Возврат Истина;

КонецФункции

// Учётная запись, серверы, порты и признак заполненности логина/пароля.
// Значения паролей в журнал не пишем - только длину, этого хватает, чтобы
// отличить пароль приложения Google (16 знаков) от обычного пароля.
Функция ПредставлениеПрофиляДляОтладки(УчетнаяЗапись, Профиль) Экспорт

	Текст = Строка(УчетнаяЗапись) + " <" + СокрЛП(УчетнаяЗапись.АдресЭлектроннойПочты) + ">"
		+ ", SMTP = " + СокрЛП(УчетнаяЗапись.SMTPСервер) + ":" + Строка(УчетнаяЗапись.ПортSMTP)
		+ ", POP3 = " + СокрЛП(УчетнаяЗапись.POP3Сервер) + ":" + Строка(УчетнаяЗапись.ПортPOP3)
		+ ", аутентификация SMTP = " + Строка(УчетнаяЗапись.ТребуетсяSMTPАутентификация)
		+ ", логин = " + СокрЛП(УчетнаяЗапись.Логин)
		+ ", логин SMTP = " + СокрЛП(УчетнаяЗапись.ЛогинSMTP)
		+ ", длина пароля = " + Строка(СтрДлина(СокрЛП(УчетнаяЗапись.Пароль)))
		+ ", длина пароля SMTP = " + Строка(СтрДлина(СокрЛП(УчетнаяЗапись.ПарольSMTP)))
		+ ", таймаут = " + Строка(УчетнаяЗапись.ВремяОжиданияСервера);

	Если Профиль <> Неопределено Тогда
		Текст = Текст + ", SSL SMTP = " + Строка(Профиль.ИспользоватьSSLSMTP)
			+ ", SSL POP3 = " + Строка(Профиль.ИспользоватьSSLPOP3);
	КонецЕсли;
...
Функция ПредставлениеСпискаУчетныхЗаписей(Список) Экспорт

	Текст = "";

	Для каждого ЭлементСписка Из Список Цикл
		Текст = Текст + ?(ПустаяСтрока(Текст), "", "; ") + Строка(ЭлементСписка.Значение);
	КонецЦикла;
...
////////////////////////////////////////////////////////////////////////////////
// ЦЕПОЧКА ПЕРЕПИСКИ И ПРОГРАММНЫЙ ОТВЕТ НА ПИСЬМО
//
// Типовая отправка кладёт в ИнтернетПочтовоеСообщение только тему, адреса и текст,
// поэтому ответ, отправленный из 1С, у получателя открывался отдельной веткой.
// Здесь заголовки In-Reply-To / References собираются по цепочке ОснованиеПисьма,
// своих реквизитов для этого не заводим - цепочка и так хранится ссылками.

// Ставит заголовки цепочки переписки для письма-ответа.
//
// Параметры
//  ПочтовоеСообщение - ИнтернетПочтовоеСообщение, письмо к отправке
//  Письмо            - ДокументСсылка.ЭлектронноеПисьмо, отправляемое письмо
//
Процедура УстановитьЗаголовкиЦепочки(ПочтовоеСообщение, Письмо) Экспорт

	Если Не ЗначениеЗаполнено(Письмо.ОснованиеПисьма) Тогда
		Возврат;
	КонецЕсли;

	Цепочка = ИдентификаторыЦепочкиПисьма(Письмо);

	Если Цепочка.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	// Ближайшее основание - последний элемент цепочки (она собрана от дальнего к ближнему).
	InReplyTo  = Цепочка[Цепочка.Количество() - 1];
	References = "";
	Для каждого Идентификатор Из Цепочка Цикл
		References = References + ?(ПустаяСтрока(References), "", " ") + Идентификатор;
	КонецЦикла;

	// Имя свойства заголовков в платформе 8.2 проверяем по факту: если его нет,
	// отправка не должна падать - письмо уйдёт просто без склейки в ветку.
	Попытка

		// В этой платформе у почтового сообщения нет свойства Заголовки — в журнале
		// это видно как «Поле об`єкту не виявлено (Заголовки)», и письма уходили без
		// склейки в ветку. Парный метод к ПолучитьПолеЗаголовка, которым мы читаем
		// In-Reply-To при приёме, работает; свойство оставляем запасным путём.
		Попытка
			ПочтовоеСообщение.УстановитьПолеЗаголовка("In-Reply-To", InReplyTo);
			ПочтовоеСообщение.УстановитьПолеЗаголовка("References", References);
		Исключение
			ПочтовоеСообщение.Заголовки.Вставить("In-Reply-To", InReplyTo);
			ПочтовоеСообщение.Заголовки.Вставить("References", References);
		КонецПопытки;

		ОтладкаПочтыВЖурнал("Цепочка: письмо = " + Строка(Письмо)
			+ ", In-Reply-To = " + InReplyTo + ", References = " + References);

	Исключение

		ОтладкаПочтыВЖурнал("Цепочка НЕ УСТАНОВЛЕНА (заголовки недоступны): письмо = " + Строка(Письмо)
			+ ". " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Истина);
...
// Message-ID писем цепочки, от самого дальнего основания к ближайшему.
// Каждый идентификатор возвращается в угловых скобках, как того требует RFC 5322.
//
// Параметры
//  Письмо - ДокументСсылка.ЭлектронноеПисьмо, письмо-ответ
//
// Возвращаемое значение:
//  Массив строк
//
Функция ИдентификаторыЦепочкиПисьма(Письмо) Экспорт

	Идентификаторы = Новый Массив;

	Текущее = Письмо.ОснованиеПисьма;
	Глубина = 0;

	Пока ЗначениеЗаполнено(Текущее) И Глубина < 20 Цикл

		Глубина = Глубина + 1;

		// Основанием может быть и Событие: у него нет реквизита ИдентификаторПисьма,
		// обращение к нему уронило бы отправку письма целиком.
		Если ТипЗнч(Текущее) <> Тип("ДокументСсылка.ЭлектронноеПисьмо") Тогда
			Прервать;
		КонецЕсли;

		Идентификатор = СокрЛП(Текущее.ИдентификаторПисьма);
		Если Не ПустаяСтрока(Идентификатор) Тогда
			Если Лев(Идентификатор, 1) <> "<" Тогда
				Идентификатор = "<" + Идентификатор + ">";
			КонецЕсли;
			Идентификаторы.Вставить(0, Идентификатор);
		КонецЕсли;

		Текущее = Текущее.ОснованиеПисьма;
...
// Создаёт письмо-ответ на существующее письмо и (по умолчанию) сразу его отправляет.
// Ответ попадает в ту же цепочку: в 1С - через ОснованиеПисьма, у получателя -
// через заголовки In-Reply-To / References.
//
// Параметры
//  ПисьмоОснование - ДокументСсылка.ЭлектронноеПисьмо, письмо, на которое отвечаем
//  Текст           - Строка, текст ответа
//  Тема            - Строка, тема; пустая - берётся тема основания с префиксом "Re: "
//  Отправлять      - Булево, отправить сразу или только записать исходящее письмо
//  Ответственный   - СправочникСсылка.Пользователи; Неопределено - текущий пользователь
//  ТекстОшибок     - Строка, сюда возвращаются ошибки отправки
//
// Возвращаемое значение:
//  ДокументСсылка.ЭлектронноеПисьмо - созданное письмо-ответ
//
Функция СоздатьОтветНаПисьмо(ПисьмоОснование, Знач Текст, Знач Тема = "", Знач Отправлять = Истина,
	Знач Ответственный = Неопределено, ТекстОшибок = "") Экспорт

	ТекстОшибок = "";

	Если Не ЗначениеЗаполнено(ПисьмоОснование) Тогда
		ТекстОшибок = "Не указано письмо-основание для ответа.";
		Возврат Документы.ЭлектронноеПисьмо.ПустаяСсылка();
	КонецЕсли;

	УчетнаяЗапись = ПисьмоОснование.УчетнаяЗапись;
	Если Не ЗначениеЗаполнено(УчетнаяЗапись) Тогда
		ТекстОшибок = "У письма " + Строка(ПисьмоОснование) + " не указана учётная запись.";
		Возврат Документы.ЭлектронноеПисьмо.ПустаяСсылка();
	КонецЕсли;
	...
	Письмо.ОтправительАдресЭлектроннойПочты = УчетнаяЗапись.АдресЭлектроннойПочты;
	Письмо.ОтправительПредставление        = УчетнаяЗапись.Наименование + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + ">";
	Письмо.ОснованиеПисьма                 = ПисьмоОснование;
	Письмо.Ответ                           = Истина;
	Письмо.Ответственный                   = Ответственный;
	Письмо.Предмет                         = ПисьмоОснование.Предмет;
	...
	Если ЗначениеЗаполнено(ПисьмоОснование.КодировкаПисьма) Тогда
		Письмо.КодировкаПисьма = ПисьмоОснование.КодировкаПисьма;
	Иначе
		Письмо.КодировкаПисьма = "utf-8";
	КонецЕсли;

	Если ПустаяСтрока(Тема) Тогда
		Тема = СокрЛП(ПисьмоОснование.Тема);
		Если НРег(Лев(Тема, 3)) <> "re:" Тогда
			Тема = "Re: " + Тема;
		КонецЕсли;
	КонецЕсли;
	Письмо.Тема = Тема;

	// Кому: на полученное письмо отвечаем отправителю, на своё исходящее - его получателям.
	ЗаполнитьПолучателейОтвета(Письмо, ПисьмоОснование);

	Если Письмо.КомуТЧ.Количество() = 0 Тогда
		ТекстОшибок = "Не удалось определить получателя ответа на письмо " + Строка(ПисьмоОснование) + ".";
		ОтладкаПочтыВЖурнал("Ответ НЕ СОЗДАН: " + ТекстОшибок, Истина);
		Возврат Документы.ЭлектронноеПисьмо.ПустаяСсылка();
	КонецЕсли;

	Письмо.КомуПредставление = ПредставлениеПолучателей(Письмо.КомуТЧ);

	Попытка
		Письмо.Записать();
	Исключение
		ТекстОшибок = "Ответ не записан: " + ОписаниеОшибки();
		ОтладкаПочтыВЖурнал("Ответ НЕ ЗАПИСАН: " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Истина);
		Возврат Документы.ЭлектронноеПисьмо.ПустаяСсылка();
	КонецПопытки;

	ОтладкаПочтыВЖурнал("Ответ создан: " + Строка(Письмо.Ссылка)
		+ ", основание = " + Строка(ПисьмоОснование)
		+ ", кому = " + Письмо.КомуПредставление
		+ ", отправлять = " + Строка(Отправлять));

	Если Отправлять Тогда

		ПисьмаКОтправке = Новый Соответствие;
		ПисьмаКОтправке.Вставить(Письмо.Ссылка, Неопределено);

		ОтправитьПисьма(ПисьмаКОтправке, Новый Соответствие, Ответственный, Ложь, ТекстОшибок);
...
// Заполняет КомуТЧ письма-ответа по письму-основанию.
Процедура ЗаполнитьПолучателейОтвета(Письмо, ПисьмоОснование) Экспорт

	Письмо.КомуТЧ.Очистить();

	Если ПисьмоОснование.СтатусПисьма = Перечисления.СтатусыПисем.Полученное Тогда

		Если Не ПустаяСтрока(ПисьмоОснование.ОтправительАдресЭлектроннойПочты) Тогда
			СтрокаКому = Письмо.КомуТЧ.Добавить();
			СтрокаКому.АдресЭлектроннойПочты = ПисьмоОснование.ОтправительАдресЭлектроннойПочты;
			СтрокаКому.Представление         = ПисьмоОснование.ОтправительИмя;
		КонецЕсли;
	...
	Если Письмо.КомуТЧ.Количество() = 0 Тогда
		...
		Для каждого СтрокаОснования Из ПисьмоОснование.КомуТЧ Цикл
			Если ПустаяСтрока(СтрокаОснования.АдресЭлектроннойПочты) Тогда
				Продолжить;
			КонецЕсли;
			СтрокаКому = Письмо.КомуТЧ.Добавить();
			СтрокаКому.АдресЭлектроннойПочты = СтрокаОснования.АдресЭлектроннойПочты;
			СтрокаКому.Представление         = СтрокаОснования.Представление;
		КонецЦикла;
...
Функция ПредставлениеПолучателей(КомуТЧ) Экспорт

	Текст = "";

	Для каждого СтрокаТЧ Из КомуТЧ Цикл
		Если ПустаяСтрока(СокрЛП(СтрокаТЧ.Представление)) Тогда
			Текст = Текст + ?(ПустаяСтрока(Текст), "", ", ") + СокрЛП(СтрокаТЧ.АдресЭлектроннойПочты);
		Иначе
			Текст = Текст + ?(ПустаяСтрока(Текст), "", ", ")
				+ СокрЛП(СтрокаТЧ.Представление) + " <" + СокрЛП(СтрокаТЧ.АдресЭлектроннойПочты) + ">";
		КонецЕсли;
	КонецЦикла;
