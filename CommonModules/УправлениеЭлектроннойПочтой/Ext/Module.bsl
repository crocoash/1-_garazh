...
//
Функция ПолучитьПисьмаПоОдному(ИнтернетПочта, УчетнаяЗапись, МассивЗаголовков) Экспорт

	Полученные   = Новый Массив;
	Обработанные = Новый Соответствие;   // ИдентификаторСообщения -> Истина
	Сбоев        = 0;

	// Ограничители, чтобы задание не висело часами на большом ящике:
	// остальное заберётся следующим заходом регламента.
	МаксимумСбоев  = 20;
	МаксимумПисем  = 100;
	...
	Пока Истина Цикл
		...
		Для Каждого Заголовок Из Заголовки Цикл
			...
			Если Полученные.Количество() >= МаксимумПисем Тогда
				Прервать;
			КонецЕсли;
		...
		Если Не ОсталосьРабота
			...
			Или Полученные.Количество() >= МаксимумПисем Тогда
			Прервать;
		КонецЕсли;
			...
			Если Получить Тогда
				...
				Если МассивЗаголовковПисем.Количество() > 0 Тогда
					...
					Исключение
						...
						МассивПисем = ПолучитьПисьмаПоОдному(ИнтернетПочта, УчетнаяЗапись, МассивЗаголовковПисем);
					КонецПопытки;
