...
Функция ПолучитьДокументОбъектИзИнтернетПочтовогоСообщения(ИнтернетПисьмо, УчетнаяЗапись = Неопределено, мРазделительАдресов = ",", ИсключениеТранзакции = Ложь)
	...
	Если Не ПустаяСтрока(ИдентификаторПисьмаОснования) Тогда
		...
		Запрос.Текст =
		// Псевдоним обязан совпадать с именем реквизита: ЗаполнитьЗначенияСвойств
		// сопоставляет колонки по имени, а реквизита «Основание» у письма нет —
		// с прежним псевдонимом входящие ответы молча оставались без основания.
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ЭлектронноеПисьмо.Ссылка КАК ОснованиеПисьма
		|ИЗ
