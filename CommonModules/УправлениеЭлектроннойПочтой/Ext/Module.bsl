...
// Ящики, почта которых забирается по IMAP вместо POP3.
//
// POP3 у Gmail отдаёт только письма, ещё не выданные по этому протоколу:
// если в тот же ящик ходит Outlook, он забирает письма первым, и для 1С их
// больше не существует. IMAP такого понятия не знает — показывает папку целиком.
//
// Список ведётся вручную и по одному ящику: переводим только те, у которых
// IMAP включён в Gmail, остальные продолжают работать по POP3 как раньше.
//
Функция ЯщикиНаIMAP() Экспорт

	Адреса = Новый Массив;
	Адреса.Добавить("khodorovska@garazh.ua");
	Адреса.Добавить("vorobey@garazh.ua");

	Возврат Адреса;

КонецФункции

// Нужно ли забирать почту этой учётной записи по IMAP.
//
Функция ПолучатьПоIMAP(УчетнаяЗапись) Экспорт

	Адрес = НРег(СокрЛП(УчетнаяЗапись.АдресЭлектроннойПочты));

	Если ПустаяСтрока(Адрес) Или УчетнаяЗапись.ПортIMAP_СД <= 0 Тогда
		Возврат Ложь;
	КонецЕсли;

	Для Каждого Разрешенный Из ЯщикиНаIMAP() Цикл
		Если Адрес = НРег(СокрЛП(Разрешенный)) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
