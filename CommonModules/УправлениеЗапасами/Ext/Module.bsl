...
Процедура СоздатьКолонкиСуммДерево(Дерево, Префикс)
	Дерево.Колонки.Добавить(Префикс + "Цена"            , Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,0)));//FOFA
	Дерево.Колонки.Добавить(Префикс + "ВалютаЦены"      , Новый ОписаниеТипов("СправочникСсылка.Валюты"));
	Дерево.Колонки.Добавить(Префикс + "ЦенаНоменклатуры", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,0)));
...
Процедура ЗаполнитьПоляСумм(СтрокаДерева, Выборка, СписокЦенОстатков, КурсыВалют)
	...
	Иначе
		ПолученнаяЦена = Ценообразование.ОкруглитьЦену(Выборка.Цена*(1+Выборка.ТипЦен.ПроцентСкидкиНаценки/100), Выборка.ТипЦен.ПорядокОкругления, Выборка.ТипЦен.ОкруглятьВБольшуюСторону);
	КонецЕсли;
...
Процедура ПолучитьДеревоОстатковНоменклатуры(ДеревоОстатков, Номенклатура, ОтображатьСклады, 
	...
	Если ПолучатьЦены Тогда
		...
		|							ТОГДА ЕСТЬNULL(ЦеныНаБазовыйТип.Цена, 0.00)
