...
Функция ЕстьДоступныеПраваДляЗапускаКонфигурации() Экспорт
	...
	Возврат РольДоступна("ПолныеПраваБезРедактирования") или РольДоступна("ПолныеПрава") ИЛИ РольДоступна("Пользователь");
...
Процедура УстановитьПараметрГраницыЗапретаИзмененияДанных() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ  РАЗРЕШЕННЫЕ
                   ...
                   |    ОбщаяДатаЗапрета.ОбщаяДата как ОбщаяДата,   
				   		   //с.д. рижий 2024-02-15 (
				   |    ТаблицаГраницыЗапрета.Объект_сд    
				   		   //с.д. рижий 2024-02-15 )
                   ...
                   //|    Организации.Организация, ОбщаяДата";         
					|    Организации.Организация, ОбщаяДата  ,ТаблицаГраницыЗапрета.Объект_сд";         
					//с.д. рижий 2024-02-15 )		   
	...
	Иначе			
		...
		Пока Выборка.Следующий() Цикл       
			...
			//стало                   
			попытка 
				Объект_сд = Выборка.Объект_сд.имя;
			Исключение
			КонецПопытки;	
			
			Если ЗначениеЗаполнено(Объект_сд) тогда
				Соответствие[сокрлп(Выборка.Организация) + ""+СокрЛП(Объект_сд)] = ?(ЗначениеЗаполнено(Выборка.ГраницаЗапретаИзменений), КонецДня(Выборка.ГраницаЗапретаИзменений), Выборка.ГраницаЗапретаИзменений);
	            Если ОбщаяДатаЗапрета = Неопределено Тогда
	                ОбщаяДатаЗапрета = ?(ЗначениеЗаполнено(Выборка.ОбщаяДата), КонецДня(Выборка.ОбщаяДата), Выборка.ОбщаяДата);
				КонецЕсли;  
		...
		Если НЕ ОбщаяДатаЗапрета = Неопределено Тогда
	    	Соответствие["ОбщаяДатаЗапретаРедактирования" + Объект_сд] = ОбщаяДатаЗапрета;
	    КонецЕсли;
		//с.д. рижий 2024-02-15 )
