...
Функция РеглОтчеты(Наим, СоздаватьИНеЗапоминать = Ложь) Экспорт
	...
	Если МассивДублей.Количество() > 0 Тогда
		Сообщить(Локализация.СтрШаблон(НСтр("ru = 'В справочнике ""Регламентированные отчеты"" обрнаружены дубли для объекта ""¤1¤""'; uk = 'У довіднику ""Регламентовані звіти"" обрнаружени дублі для об''єкта ""¤1¤""'"), СокрП(Наим)), СтатусСообщения.Информация);
		Для каждого СсылкаДубль Из МассивДублей Цикл
			Попытка
				...
				Сообщить(Локализация.СтрШаблон(НСтр("ru = 'Элемент ""¤1¤"" (¤2¤) помечен на удаление'; uk = 'Елемент ""¤1¤"" (¤2¤) позначений на видалення'"), Строка(СсылкаДубль), Строка(СсылкаДубль.Код)), СтатусСообщения.БезСтатуса);
			Исключение
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ДЛЯ РАБОТЫ С 1С-ЗВИТ

// Функция выполняет запуск  1С-Звит"
//
Функция ЗапуситьЗвит1С() Экспорт
	...
	// Выведем причины невозможности использования 1С-Звит, в случае если менеджер не инициализирован
...
// Функция выполняет запуск запуск пункта меню "Выгрузка налоговых документов в 1С-Звит"
//
...
// Функция выполняет запуск пункта меню "Проверка входящих налоговых документов в "1С-Звит"
//
