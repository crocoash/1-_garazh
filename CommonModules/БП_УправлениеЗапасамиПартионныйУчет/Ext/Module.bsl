	...
	Если СтрокаДереваПартий <> Неопределено Тогда
		...
		Для Каждого СтрокаПартииРаспределения ИЗ СтрокаДереваПартий.Строки Цикл
			...
			Если НЕ СтрокаПартии.Количество > 0 Тогда
				Продолжить;
			КонецЕсли; 
			Если Парт <> 0 Тогда   //FOFA
				Если ЗначениеЗаполнено(Парт) Тогда
				если Парт <> СтрокаПартии.ДокументОприходования Тогда
						Продолжить;
					КонецЕслИ;
				КонецЕслИ;
			КонецЕслИ;
...
Процедура ВыполнитьСписание(СтруктураПараметров, ТаблицаСписания, МоментКон, Останавливаться = Ложь, ПроведениеОстановлено = Ложь )
	...
	Документы.ПереоценкаЗапасов_СД.УдалитьВсе(СтруктураПараметров.Регистратор);
	
	ТаблицаДвиженийПартииТоваровНаСкладахУпр = СтруктураПараметров.ТаблицаДвиженийПартииТоваровНаСкладахУпр; 
	ТЗСкидокПоУсловиюПоставки = СтруктураПараметров.Регистратор.Сделка.Товары.выгрузить(,"Номенклатура,БП_ПроцентСкидки, БП_ЦенаПоУсловиюПоставки, Количество,БП_СуммаПоУсловиюПоставки");     
	
	
	//	ТЗСкидокПоУсловиюПоставки = СтруктураПараметров.Регистратор.Сделка.Товары.выгрузить(,"Номенклатура,БП_ПроцентСкидки, БП_ЦенаПоУсловиюПоставки, БП_СуммаПоУсловиюПоставки");
	//
	//
	//Для Каждого Стр Из ТЗСкидокПоУсловиюПоставки Цикл
	//	ТЗСкидокПоУсловиюПоставки.Удалить(Стр);
	//КонецЦикла;


	//
	//ТЗСкидокПоУсловиюПоставкиОтбор = ТЗСкидокПоУсловиюПоставки.СкопироватьКолонки();
	...
	//		НовСтрока = ТЗСкидокПоУсловиюПоставкиОтбор.Добавить();
	...
	ТаблицаДвиженийПартииТоваровНаСкладахУпрДокОприходования = ТаблицаДвиженийПартииТоваровНаСкладахУпр.Скопировать(,"ДокументОприходования");
	ТаблицаДвиженийПартииТоваровНаСкладахУпрДокОприходования.Свернуть("ДокументОприходования");                            
	МассивПоступлений = ТаблицаДвиженийПартииТоваровНаСкладахУпрДокОприходования.ВыгрузитьКолонку("ДокументОприходования");
	
	Для каждого ДокументОприходования из МассивПоступлений цикл
		ДокПереоценка = Документы.ПереоценкаЗапасов_СД.СоздатьДокумент();
		ДокПереоценка.Дата = СтруктураПараметров.Период - 1;
		
		Если ДокументОприходования.Метаданные().Реквизиты.Найти("Контрагент") <> Неопределено Тогда
			
			Если ДокументОприходования.Контрагент <> Неопределено Тогда
				ДокПереоценка.Контрагент = ДокументОприходования.Контрагент;
				ДокПереоценка.ДоговорКонтрагента = ДокументОприходования.ДоговорКонтрагента; 
				
			КонецЕсли;
			
		КонецЕсли;
		ДокПереоценка.Комментарий = "#Створено автоматично на підставі переоцінки по умовам поставки " + Строка(СтруктураПараметров.Регистратор.Сделка);
		ДокПереоценка.БП_Основание = СтруктураПараметров.Регистратор;
		Отбор = Новый Структура();
		Отбор.Вставить("ДокументОприходования",ДокументОприходования);
		Строки = ТаблицаДвиженийПартииТоваровНаСкладахУпр.НайтиСтроки(Отбор);
		
		Для каждого СтрокаДвижения из Строки цикл       
			                                              
			НайденнаяСтрока = ТЗСкидокПоУсловиюПоставкиОтбор.Найти(СтрокаДвижения.Номенклатура, "Номенклатура");
			Если НайденнаяСтрока = Неопределено Тогда
				Продолжить;
			Иначе
				НоваяЦена = НайденнаяСтрока.БП_СуммаПоУсловиюПоставки / НайденнаяСтрока.Количество * СтрокаДвижения.Количество; //.БП_ЦенаПоУсловиюПоставки; //	
			КонецЕсли;
			
			НовСтрока = ДокПереоценка.Товары.Добавить();
			НовСтрока.Номенклатура = СтрокаДвижения.Номенклатура;
			НовСтрока.ЦенаПартииНовая = НайденнаяСтрока.БП_СуммаПоУсловиюПоставки;//НоваяЦена;
			
			НовСтрока = ДокПереоценка.ОстаткиПартии.Добавить();
			НовСтрока.Номенклатура = СтрокаДвижения.Номенклатура;
			//НовСтрока.ЦенаПартииНовая = 1600;   
			НовСтрока.Склад = СтрокаДвижения.Склад;
			НовСтрока.ДокументОприходования = ДокументОприходования;
			НовСтрока.СтатусПартии = СтрокаДвижения.СтатусПартии;
			НовСтрока.Качество  = СтрокаДвижения.Качество;
			НовСтрока.Количество =  СтрокаДвижения.Количество;
			НовСтрока.Стоимость =  СтрокаДвижения.Стоимость;
			НовСтрока.СтоимостьНовая = НоваяЦена;  // НовСтрока.Количество * НоваяЦена;
            НовСтрока.СтоимостьОтличие = НовСтрока.СтоимостьНовая - НовСтрока.Стоимость;
		КонецЦикла;
