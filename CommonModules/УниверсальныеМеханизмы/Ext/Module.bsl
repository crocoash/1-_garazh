...
Функция НапечататьВнешнююФорму(Ссылка, ИсточникМакета) Экспорт
	...
	Попытка
		...
		Если ДополнительныеПараметры <> Неопределено Тогда
			...
			Если Обработка.Метаданные().Реквизиты.Найти("ДопПараметры") <> Неопределено Тогда   
				ДопПараметры = Новый ХранилищеЗначения(ДополнительныеПараметры);
				Обработка.ДопПараметры = ДопПараметры;
			КонецЕсли;
		КонецЕсли;
