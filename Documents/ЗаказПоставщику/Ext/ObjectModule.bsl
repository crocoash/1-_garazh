...
//+БП: claude (27.08.2026) <>
// Дата производства и страна приезжают с предзаказа в строки товаров заказа покупателя.
// В заказе поставщику колонок под них нет, поэтому кладём их в поле "Дополнение"
// (реквизит ДополнениеКАдресуДоставки) одной строкой вида "2024 Китай".
// Разные значения по строкам заказа перечисляются через "; ".
//
Процедура ЗаполнитьДополнениеПоДатеПроизводстваИСтране(Основание)
	...
	Для Каждого ТекСтрока Из Основание.Товары Цикл
		...
		Значение = СокрЛП(СокрЛП(ТекСтрока.ДатаПроизводства) + " " + СокрЛП(ТекСтрока.Страна));
		Если ПустаяСтрока(Значение) Тогда
			Продолжить;
		КонецЕсли;

		Если Значения.Найти(Значение) = Неопределено Тогда
	...
	Для Каждого Значение Из Значения Цикл
		Текст = Текст + ?(ПустаяСтрока(Текст), "", "; ") + Значение;
	КонецЦикла;
