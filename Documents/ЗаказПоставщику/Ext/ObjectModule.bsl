...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	БП_ОбщийМодуль.ЗаполнитьВидЗаказаПоставщикуПоТекущемуПользователю(ЭтотОбъект);
	//-БП: @Cvet_kov (27.08.2026) <>
	...
	// +fofa + 13.09.2021 20:32:28
	Количество = Товары.Итог("Количество");
	Состав = "";
	Для каждого Стр Из Товары Цикл
		Состав = Состав + Строка(Стр.Номенклатура.Наименование)+", ";
	КонецЦикла; 
	// -fofa
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//С.Д. Сеня 13,12,23 +
	Если ЗначениеЗаполнено(Основной_Контрагент_СД) Тогда    
		Если не ЗначениеЗаполнено(ОсновнойДоговорКонтрагента_СД) Тогда
			Отказ = Истина;
			#Если Клиент Тогда  
				Сообщить("С.Д. Не заполнен 'Основной договор контрагента'! закладка - 'Доп.'!");
			#КонецЕсли  
		КонецЕсли;	
	Иначе
		Если ЗначениеЗаполнено(ОсновнойДоговорКонтрагента_СД) Тогда 
			 ОсновнойДоговорКонтрагента_СД = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
		КонецЕсли;	
	КонецЕсли;	
	//С.Д. Сеня 13,12,23 -
	...
	//			Если ДокументОснование.Статус = Справочники.Статусы.Создан Тогда
	//				ЗАк = ДокументОснование.ПолучитьОбъект();
	//				ЗАк.Статус = Справочники.Статусы.НайтиПоКоду("000000052");
	//				Зак.Записать();
	//			КонецЕслИ;
	//		КонецЕслИ;
	//	КонецЕслИ;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		БП_ЗаполнитьОрганизациюПоНастройкеАвтозаполнения(Основание.Организация);
		//-БП: d.matyash (22.09.2025) <>		

		//+БП: @Cvet_kov (27.08.2026) <>
		ЗаполнитьДополнениеПоДатеПроизводстваИСтране(Основание);
		//-БП: @Cvet_kov (27.08.2026) <>

		//+БП: @Cvet_kov (28.08.2026) <>
		БП_ОбщийМодуль.ЗаполнитьПриоритетБезналичный(ЭтотОбъект);
		//-БП: @Cvet_kov (28.08.2026) <>
	...
	//+БП: d.matyash (17.03.2024)<>
	БП_УстановитьЗаписатьНомерПапки();
	//-БП: d.matyash (17.03.2024)<>
...
//+БП: @Cvet_kov (28.08.2026) <>
// Дата производства и страна заполняются менеджером в табличной части предзаказа.
// В заказ покупателя они копируются один раз, в момент его создания
// (СД_Програмист.СоздатьЗаказНаОснованииПредЗаказа), и при обновлении предзаказа
// с сайта уже не переписываются - поэтому читаем сам предзаказ через основание,
// а на товары заказа откатываемся, только если предзаказа нет (заказ заведён руками).
//
// В заказе поставщику колонок под них нет, поэтому кладём их в поле "Дополнение"
// (реквизит ДополнениеКАдресуДоставки) строкой вида "Шина 205/55 R16 - 2024 Китай".
// Берутся только те позиции, которые реально попали в этот заказ поставщику:
// табличная часть может быть заполнена необеспеченной частью заказа, то есть не всеми
// строками. Разные позиции перечисляются через "; ", повторы схлопываются.
//
Процедура ЗаполнитьДополнениеПоДатеПроизводстваИСтране(Основание) Экспорт

	//+БП: @Cvet_kov (28.08.2026) ВРЕМЕННАЯ ОТЛАДКА <>
	// Пишем ход заполнения в журнал регистрации, событие "ЗаказПоставщику.Дополнение".
	// Разбираемся, почему у заказов поставщику поле остаётся пустым. Пользователю ничего
	// не показывается. Убрать вместе с этой пометкой, когда причина будет понятна.
	Попытка
		Отладка = "Основание: " + Строка(ТипЗнч(Основание)) + " " + Строка(Основание)
			+ "; Проведен: " + Строка(Основание.Проведен)
			+ "; ПредЗаказ: " + Строка(Основание.ПредЗаказ)
			+ "; строк в Товары заказа поставщику: " + Строка(Товары.Количество())
			+ "; строк в Товары основания: " + Строка(Основание.Товары.Количество());
	Исключение
		Отладка = "шапку отладки собрать не удалось: " + ОписаниеОшибки();
	КонецПопытки;
	//-БП: @Cvet_kov (28.08.2026) ВРЕМЕННАЯ ОТЛАДКА <>

	Если ЗначениеЗаполнено(Основание.ПредЗаказ) Тогда
		СтрокиИсточника = Основание.ПредЗаказ.Товары;
		Отладка = Отладка + "; источник: предзаказ";
	Иначе
		СтрокиИсточника = Основание.Товары;
		Отладка = Отладка + "; источник: товары основания";
	КонецЕсли;
	Отладка = Отладка + ", строк в источнике: " + Строка(СтрокиИсточника.Количество());

	// что реально попало в этот заказ поставщику
	НоменклатураЗаказа = Новый Соответствие;
	СписокЗаказа = "";
	Для Каждого ТекСтрокаЗаказа Из Товары Цикл
		НоменклатураЗаказа.Вставить(ТекСтрокаЗаказа.Номенклатура, Истина);
		СписокЗаказа = СписокЗаказа + ?(ПустаяСтрока(СписокЗаказа), "", ", ") + Строка(ТекСтрокаЗаказа.Номенклатура);
	КонецЦикла;
	Отладка = Отладка + Символы.ПС + "Номенклатура заказа поставщику: " + ?(ПустаяСтрока(СписокЗаказа), "<пусто>", СписокЗаказа);

	Текст = "";
	Пропущено = "";
	Учтенные = Новый Соответствие;

	Для Каждого ТекСтрока Из СтрокиИсточника Цикл

		Если НоменклатураЗаказа.Получить(ТекСтрока.Номенклатура) = Неопределено Тогда
			Пропущено = Пропущено + Символы.ПС + "  " + Строка(ТекСтрока.Номенклатура) + " - нет в заказе поставщику";
			Продолжить;
		КонецЕсли;

		ДатаИСтрана = СокрЛП(СокрЛП(ТекСтрока.ДатаПроизводства) + " " + СокрЛП(ТекСтрока.Страна));
		Если ПустаяСтрока(ДатаИСтрана) Тогда
			Пропущено = Пропущено + Символы.ПС + "  " + Строка(ТекСтрока.Номенклатура) + " - дата и страна пустые";
			Продолжить;
		КонецЕсли;

		Фрагмент = СокрЛП(Строка(ТекСтрока.Номенклатура)) + " - " + ДатаИСтрана;
		Если Учтенные.Получить(Фрагмент) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Учтенные.Вставить(Фрагмент, Истина);

		Текст = Текст + ?(ПустаяСтрока(Текст), "", "; ") + Фрагмент;

	КонецЦикла;

	//+БП: @Cvet_kov (28.08.2026) ВРЕМЕННАЯ ОТЛАДКА <>
	Попытка
		Отладка = Отладка + Символы.ПС + "Пропущено:" + ?(ПустаяСтрока(Пропущено), " нет", Пропущено)
			+ Символы.ПС + "Итог: " + ?(ПустаяСтрока(Текст), "<пусто, поле не меняем>", Текст);
		ЗаписьЖурналаРегистрации("ЗаказПоставщику.Дополнение",
			УровеньЖурналаРегистрации.Информация, , , Отладка);
	Исключение
	КонецПопытки;
	//-БП: @Cvet_kov (28.08.2026) ВРЕМЕННАЯ ОТЛАДКА <>
	...
	ДополнениеКАдресуДоставки = Текст;
...
Процедура БП_УстановитьЗаписатьНомерПапки()
	//+БП: d.matyash(18.03.2024) <>
	УстановитьПривилегированныйРежим(Истина);
	
	БП_ПутьКПапкеЗаказовПоставщиков = Константы.БП_ПутьКПапкеЗаказовПоставщиков.Получить();
	Если НЕ ЗначениеЗаполнено(БП_ПутьКПапкеЗаказовПоставщиков) тогда
		Сообщить("Не заповнена константа ""Шлях до папки заказів поставщиків""! Введіть дані для коректної роботи програми.");
		Возврат;
	КонецЕсли;
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      |	БП_РезервНомеровПапок.НомерПапки КАК НомерПапки
	                      |ИЗ
	                      |	РегистрСведений.БП_РезервНомеровПапок КАК БП_РезервНомеровПапок
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	БП_РезервНомеровПапок.НомерПапки УБЫВ");
	
	ТЗНомер = Запрос.Выполнить().Выгрузить();
		
	Если ТЗНомер.Количество() > 0 тогда
		ТекущийНомер = ТЗНомер[0].НомерПапки;
		ТекущийНомер = ТекущийНомер + 1;
	КонецЕсли;
	
	БП_НомерПапки = ТекущийНомер;
	...
	МенеджерЗаписи = РегистрыСведений.БП_РезервНомеровПапок.СоздатьМенеджерЗаписи();	
	МенеджерЗаписи.Документ = СсылкаНаОбъект;
	МенеджерЗаписи.НомерПапки = БП_НомерПапки;		
	МенеджерЗаписи.Записать();
	
	ПутьКЗаказу = БП_ПутьКПапкеЗаказовПоставщиков + "\" + Строка(БП_НомерПапки);
	СоздатьКаталог(ПутьКЗаказу + "\Реквізити");
	СоздатьКаталог(ПутьКЗаказу + "\Платіжки");
	
	УстановитьПривилегированныйРежим(Ложь);
	//-БП: d.matyash(18.03.2024) <>
КонецПроцедуры

Процедура БП_ЗаполнитьОрганизациюПоНастройкеАвтозаполнения(ОрганизацияЗаказаПокупателя)
	
	УстановитьПривилегированныйРежим(Истина);
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	БП_НастройкиАвтозаменыОрганизацииВЗаказеПоставщикуОрганизации.Ссылка,
	                      |	БП_НастройкиАвтозаменыОрганизацииВЗаказеПоставщикуОрганизации.ОрганизацияЗаказаПоставщику
	                      |ИЗ
	                      |	Справочник.БП_НастройкиАвтозаменыОрганизацииВЗаказеПоставщику.Организации КАК БП_НастройкиАвтозаменыОрганизацииВЗаказеПоставщикуОрганизации
	                      |ГДЕ
	                      |	БП_НастройкиАвтозаменыОрганизацииВЗаказеПоставщикуОрганизации.Ссылка.Активное
	                      |	И НЕ БП_НастройкиАвтозаменыОрганизацииВЗаказеПоставщикуОрганизации.Ссылка.ПометкаУдаления
	                      |	И БП_НастройкиАвтозаменыОрганизацииВЗаказеПоставщикуОрганизации.ОрганизацияЗаказаПокупателя = &ОрганизацияЗаказаПокупателя");
	
	
	Запрос.УстановитьПараметр("ОрганизацияЗаказаПокупателя", ОрганизацияЗаказаПокупателя);
	...
	Если НЕ ТЗ.Количество() тогда
		Возврат;
	Иначе
		Организация = ТЗ[0].ОрганизацияЗаказаПоставщику;		
	КонецЕсли;
