...
//Если Организация.Код <>  "000000001"  Тогда
	...
	Если Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда//FOFA
	...
	Марка = ЭтотОбъект.БП_Марка;
	Модель = ЭтотОбъект.БП_Модель;
	ВинКод = ЭтотОбъект.БП_НомерВинКода;
	НомернойЗнак = ЭтотОбъект.БП_НомернойЗнак;
	...
	|	ВзаиморасчетыСКонтрагентамиОбороты.СуммаВзаиморасчетовРасход - ВзаиморасчетыСКонтрагентамиОбороты.СуммаВзаиморасчетовПриход КАК Аванс
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыСКонтрагентами.Обороты(, , , Сделка = &Заказ) КАК ВзаиморасчетыСКонтрагентамиОбороты";
	АвансЗапрос.УстановитьПараметр("Заказ",Ссылка);
	Результат = АвансЗапрос.Выполнить().Выбрать();
	Пока Результат.Следующий() Цикл
		Если Результат.Аванс > 0 Тогда
		ОбластьПробел = Макет.ПолучитьОбласть("Пробел1");
		ОбластьПробел.Параметры.ЗначениеПараметра = Строка(Результат.Аванс) + " грн.";
		ТабДокумент.Вывести(ОбластьПробел);
		Прервать; 
		КонецЕсли;
	КонецЦикла;
		ОбластьМакета = Макет.ПолучитьОбласть("Магазин");		
		ОбластьМакета.Параметры.Наименование = "GARAZH.UA Шины | Диски | Сервис";
		ОбластьМакета.Параметры.Адрес = "м.Київ, вул. Лугова 12";
		ОбластьМакета.Параметры.Телефон = Подразделение.Телефон;
		ОбластьМакета.Параметры.Факс = "(044) 221 81 02";
		ОбластьМакета.Параметры.Телефон = "(067) 109 67 33, (099) 109 67 33, (063) 109 67 33";		
		ОбластьМакета.Параметры.Сайт = "garazh.ua";
		ОбластьМакета.Параметры.Почта = "CustomerService@garazh.ua";
	...
	КонецЕсли;
	Если СокрЛП(Организация.Код) = "000000001" Тогда//fofa
	...
	Если Шапка.УчитыватьНДС Тогда
		...
		ОбластьМакета.Параметры.НДС      = ?(Шапка.СуммаВключаетНДС, НСтр("ru='В том числе НДС:';uk='У тому числі ПДВ:'",КодЯзыкаПечать), НСтр("ru='Сумма НДС:';uk='Сума ПДВ:'",КодЯзыкаПечать));
			Если СокрЛП(Организация.Код) <> "000000001" Тогда//fofa
			ОбластьМакета.Параметры.НДС      = "Без ПДВ";
			КонецЕслИ;
		ТабДокумент.Вывести(ОбластьМакета);
	...
	ОбластьМакета.Параметры.ИтоговаяСтрока = НСтр("ru='Всего наименований ';uk='Всього найменувань '",КодЯзыкаПечать) + ЗапросТовары.Количество() + "," +
											 ...
											 Если СокрЛП(Организация.Код) <> "000000001" Тогда//fofa
												 ОбластьМакета.Параметры.СуммаПрописью  = "Без ПДВ";
											 КонецЕслИ;
	ОбластьМакета.Параметры.До = ФОрмат(Дата+24*60*60,"ДФ=dd.MM.yyyy");//fofa											 
	ТабДокумент.Вывести(ОбластьМакета);
...
//	КонецЕслИ;
	...
	Для каждого ВыборкаСтрокТовары из ЗапросТовары Цикл 
		...
		ОбластьМакета.Параметры.Товар       = СокрЛП(ВыборкаСтрокТовары.Номенклатура.НаименованиеПолное) + ФормированиеПечатныхФорм.ПредставлениеСерий(ВыборкаСтрокТовары);
		Если ВыводитьКоды Тогда
			ОбластьМакета.Параметры.КодАртикул = ВыборкаСтрокТовары.КодАртикул;
		КонецЕсли;
	...
	//с.д.(
	Если Тип = "Счет" Тогда
		ОбластьМакета = Макет.ПолучитьОбласть("Инфо_СД");
		ТабДокумент.Вывести(ОбластьМакета);
	КонецЕсли;
	//с.д.)
	Возврат ТабДокумент;
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	//+БП: d.matyash (18.08.2025) <>  
	ИначеЕсли ИмяМакета = "АвтоПечать" Тогда
		ПФ = ОбщегоНазначения.ПолучитьШаблонАвтоПечать(,Организация, ФормаРасчета, Подразделение);
		Если ПФ = Неопределено тогда 
			Возврат;
		КонецЕсли;
		
		СтруктураРасшифровки = Новый Структура("ВидПечатнойФормы, СсылкаНаВнешнююОбработку, ЗаменяемаяПечатнаяФорма, НомерСтроки", Перечисления.ВидыДополнительныхВнешнихОбработок.ПечатнаяФорма,ПФ, Неопределено, 1);
		ТабДокумент = УниверсальныеМеханизмы.НапечататьВнешнююФорму(Ссылка, СтруктураРасшифровки);
	//-БП: d.matyash (18.08.2025) <> 	
	...
	КонецЕсли;
	Если Организация.Код = "000000001" Тогда
		Если ПройтиПроверку() = ЛОжь Тогда
			Возврат;
		КонецЕсли; 
	КонецЕсли; 
...
Функция ПройтиПроверку()
	Если Не ЗначениеЗаполнено(Контрагент.НаименованиеПолное) Тогда
		Сообщить("Заполните полное название  контрагента",СтатусСообщения.Важное);
		Возврат Ложь;
	КонецЕсли; 	
	
	//Если Не ЗначениеЗаполнено(Контрагент.ИНН) Тогда
	//	Сообщить("Заполните ИНН контрагента",СтатусСообщения.Важное);
	//	Возврат Ложь;
	//КонецЕсли; 	
	
	Если Не ЗначениеЗаполнено(Контрагент.КодПоЕДРПОУ) Тогда
		Сообщить("Заполните ЕДРПОУ контрагента",СтатусСообщения.Важное);
		Возврат Ложь;
	КонецЕсли; 	
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   Телефонклієнта = Ложь;
				   //Телефондлядокументів = Ложь;
				   Електроннапошта = Ложь;
				   Юридичнаадреса = Ложь;
				   Фізичнаадреса = Ложь;
				   Пока Рез.Следующий() Цикл
					   
					   Если ТипЗнч(Рез.Вид) <> Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
						   Продолжить;
					   КонецЕсли;
					   
	
					   Если Рез.Вид.Код = "000000002" Тогда
						   Фізичнаадреса = Истина;
					   КонецЕсли; 
					   Если Рез.Вид.Код = "000000001" Тогда
						   Юридичнаадреса = Истина;
					   КонецЕсли; 
					   Если Рез.Вид.Код = "000000013" Тогда
						   Електроннапошта = Истина;
					   КонецЕсли; 
					   Если Рез.Вид.Код = "000000002" Тогда
						   // Телефондлядокументів = Истина;
					   КонецЕсли; 
					   Если Рез.Вид.Код = "000000005" Тогда
						   Телефонклієнта = Истина;
					   КонецЕсли; 
					   
				   КонецЦикла; 
Если Телефонклієнта = Ложь Тогда
	Сообщить("Заполните телефон контрагента",СтатусСообщения.Важное);
	Возврат Ложь;
	
КонецЕсли; 
Если Електроннапошта = Ложь Тогда
	Сообщить("Заполните электронную почту контрагента",СтатусСообщения.Важное);
	Возврат Ложь;
КонецЕсли; 
Если Юридичнаадреса = Ложь Тогда
	Сообщить("Заполните юридический адрес контрагента",СтатусСообщения.Важное);
	Возврат Ложь;
КонецЕсли; 
Если Фізичнаадреса = Ложь Тогда
	Сообщить("Заполните физический адрес контрагента",СтатусСообщения.Важное);
	Возврат Ложь;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	//СтруктураПечатныхФорм = Новый Структура("Заказ,ЗаказКорректировка,Счет,Бланк,АвтоПечать",
	//                                        "Заказ покупателя",
	//                                        "Заказ покупателя (с учетом корректировок)",
	//                                        "Счет на оплату (с учетом корректировок)",
	//                                        "Бланк товарного наполнения",
	//										"Автодрук замовлення покупця");
	
	СтруктураПечатныхФорм = Новый Структура("Бланк,АвтоПечать,Заказ",
											"Бланк товарного наполнения",
											"Автодрук счет фактура / заказ покупателя",
											"Заказ покупателя (только для касиров)");
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	КонецЕсли;
//FOFA
	Если Статус <> Справочники.Статусы.НайтиПоКоду("000000013") Тогда
...
//с.д. Рижий 2023-01-30 (   

Функция _ПолучитьЦенаПокупки(Номенклатура) 
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.ПартииТоваровНаСкладах.ОстаткиИОбороты(&ПериодДата, &ПериодДата, Авто, ДвиженияИГраницыПериода, Номенклатура = &Номенклатура) КАК ПартииТоваровНаСкладахОстаткиИОбороты";
	
	Запрос.УстановитьПараметр("Номенклатура", Номенклатура);
	Запрос.УстановитьПараметр("ПериодДата", Дата);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если  ВыборкаДетальныеЗаписи.КоличествоКонечныйОстаток = 0 тогда 
			РЕзультат = 0;	
		Иначе	
			РЕзультат =   ВыборкаДетальныеЗаписи.СтоимостьКонечныйОстаток/  ВыборкаДетальныеЗаписи.КоличествоКонечныйОстаток;
		конецЕсли;	
...
Функция ДвижениеПродажиПлан_сд(РежимПроведения,СтруктураШапкиДокумента,ТаблицаПоТоварам,ТаблицаПоУслугам,Отказ,Заголовок)
	
	
		// ТОВАРЫ ПО РЕГИСТРУ Продажи.

			НаборДвижений = Движения.ПродажиПлан_сд;
			...
			//	
			//	УправлениеПроектами.ОтразитьДвиженияПоПроектам(ТаблицаПоТоварам,ТаблицаДвижений, , Дата, "Продажи");
			//	
			//Иначе
			...
			для каждого строкатабчасти из ТаблицаДвижений цикл
				
				Если строкатабчасти.ЦенаПокупки = 0 тогда 
					
					 строкатабчасти.ЦенаПокупки = _ПолучитьЦенаПокупки(строкатабчасти.Номенклатура); 
					 строкатабчасти.СуммаПокупки = строкатабчасти.ЦенаПокупки * строкатабчасти.Количество;
	...
	//		ТаблицаДвижений.ЗаполнитьЗначения(Ссылка,"ДокументПродажи");
			...
			//	Для каждого Ст из Сделка.Товары Цикл
			//		Если Ст.Номенклатура = Стр.Номенклатура и Ст.Количество = Стр.Количество Тогда
			//			//Попытка
			...
			//			КурсВ = МодульВалютногоУчета.ПолучитьКурсВалюты(ст.ВалютаЗакупкиПодразделение,Дата);
			//			Попытка
			//			Курс = курсВ.Курс/КурсВ.Кратность;
			//		Исключение
			//			Курс = 1;
			//			КонецПопытки;
			//			Движение.СтоимостьПодразделенияОплатаЗатрат = Ст.ЦенаЗакупкиПодразделение*Курс*Ст.Количество+Сделка.ОплатаЗатрат*(Ст.Сумма/Сделка.Товары.Итог("Сумма"));
			...
			//
			Если Не Отказ Тогда
				Движения.ПродажиПлан_сд.ВыполнитьДвижения();
			КонецЕсли;
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	//с.д. Рижий 2023-01-30 (
	ДвижениеПродажиПлан_сд(РежимПроведения,СтруктураШапкиДокумента,ТаблицаПоТоварам,ТаблицаПоУслугам,Отказ,Заголовок);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// Сумма могла измениться после выбора формы расчёта - проверяем лимиты
	// (таблица ОрганизацииПоСумме в карточке формы расчёта).
	БП_ОбщийМодуль.ПересчитатьОрганизациюПоСумме(ЭтотОбъект);
	//-БП: @Cvet_kov (02.09.2026) <>
	...
	КонецЦикла;
	#Если Клиент Тогда
		Если Не Ссылка.ПометкаУдаления и ПометкаУдаления и Статус.Код = "000000008" Тогда
			Сообщить("ЗАпрещено устанавливать пометку на удаление на заказ в статусе <Отменен авт.> ");
			Отказ = Истина;
		КонецЕсли; 
	#КонецЕслИ
	 СуммаБонуса = 0;
	 Если ЗначениеЗаполнено(ПриведиДруга) Тогда
		 Для каждого Стр из Услуги Цикл
			 Если Стр.Номенклатура.БонусПлюс Тогда
				 СуммаБонуса = СуммаБонуса + Стр.Сумма * Стр.Номенклатура.ПроцентБонуса/100;
			 КонецЕсли; 
		 КонецЦикла;
	 КонецЕслИ;  
	 
	 
	 //+БП: d.matyash (27.11.2025) <>	
	 НоваяСтрока = БП_ИсторияЗаписиДокумента.Добавить(); 
...
Процедура  ПолучитьРеквизитыНаценки(Н,К,ТЦ)
	Сумма1 = 0;
	Сумма2 = 0;
	Наценка = 0;
	Запрос = Новый Запрос;
	...
	|	И ПараметрыРасчетаЦенНоменклатуры.Поставщик = &Поставщик
	|	И ПараметрыРасчетаЦенНоменклатуры.ТицЦены = &ТЦ";
	Запрос.УстановитьПараметр("Диаметр",Н.ДиаметрДиска);
	Запрос.УстановитьПараметр("Сезон",Н.Сезонность);
	Запрос.УстановитьПараметр("Поставщик",К);
	Запрос.УстановитьПараметр("ТЦ",ТЦ);
	Рез = Запрос.Выполнить().Выбрать();
	Пока Рез.Следующий() Цикл
		Сумма1 = Рез.Сумма1;
		Сумма2 = Рез.Сумма2;		   
...
Процедура ИзменитьЦену(Н,Т)
		...
		|				И Номенклатура В ИЕРАРХИИ (&Ном)) КАК ЦеныНоменклатурыКонтрагентовСрезПоследних
		|
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура,
		|	Валюта,
		...
		ЗапросЦен.УстановитьПараметр("ДатаСтарта",Дата-300*24*60*60);
		ЗапросЦен.УстановитьПараметр("Ном",Н);
		РезЦ = ЗапросЦен.Выполнить().Выбрать();
		РезЦ.Сбросить();
		Стр = Новый Структура;
		...
		Если Цена <> 0 Тогда
			...
			Движение.Период = Дата;
			Движение.ТипЦен = Т;
			Движение.Номенклатура = Н.Ссылка;
			Движение.Валюта = Валюта;
			Движение.ЕдиницаИзмерения = Н.ЕдиницаХраненияОстатков;
			ПолучитьРеквизитыНаценки(Н.Ссылка,Контрагент,Т);
			Движение.Цена = ОКР((Цена +Сумма1/МодульВалютногоУчета.ПолучитьКурсВалюты(Валюта,Дата).курс*МодульВалютногоУчета.ПолучитьКурсВалюты(Валюта,Дата).кратность) * (1+Наценка/100)+сумма2/МодульВалютногоУчета.ПолучитьКурсВалюты(Валюта,Дата).курс*МодульВалютногоУчета.ПолучитьКурсВалюты(Валюта,Дата).кратность,0);
			Движение.Поставщик = Контрагент;
			Сообщить("Изменили цену для "+Н.Ссылка);
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//			Если Строка.Размещение <> СкладГруппа Тогда
	//				Если Не ЗначениеЗаполнено(Строка.Перемещение) Тогда
	//					Если Статус <> Справочники.Статусы.НайтиПоКоду("000000013") Тогда
	//					Сообщить("По "+СокрЛП(Строка.Номенклатура.Наименование)+" не введено перемещение!");
	//					Отказ = Истина;
	//					КонецЕсли;
	//				КонецЕслИ;
	//			КонецЕслИ;
	...
	//	КонецЕсли;
	//КонецЦикла;
	Если Статус = Справочники.Статусы.НайтиПоКоду("000000009").Ссылка или Статус = Справочники.Статусы.НайтиПоКоду("000000007").Ссылка Тогда
		Сообщить("Запрещено проводить заказ в данном статусе");
		Отказ = Истина;
		Возврат;
	КонецЕслИ;
	...
	//с.д. Рижий 2023-01-30 (    
	СтруктураПолей.Вставить("СуммаПокупки"                       , "СуммаПокупки");
	СтруктураПолей.Вставить("ценаПокупки"                       , "ценаПокупки");
	...
	|ИЗ
	|	РегистрНакопления.ПартииТоваровНаСкладах.Остатки(, Номенклатура В (&Ном)) КАК ТоварыНаСкладахОстатки
	...
	Для каждого Строка Из Товары Цикл
		СписНом.Добавить(Строка.Номенклатура);
	КонецЦикла;
	
	ЗапросОстатки.УстановитьПараметр("Ном",СписНом);
	РезОстатки = ЗапросОстатки.Выполнить().Выбрать();
	СписНомБезОст = Новый СписокЗначений;
	есть = ложь;
	Для Каждого Строка Из Товары Цикл
		РезОстатки.Сбросить();
		Стр = Новый Структура;
		Стр.Вставить("Номенклатура",Строка.Номенклатура);
		Ост = 0;
		Пока РезОстатки.НайтиСледующий(Стр) Цикл
			Ост = РезОстатки.Резерв;
		КонецЦикла;
		Если Ост<=0 Тогда
			Есть = Истина;
			СписНомБезОст.Добавить(Строка.номенклатура);
		КонецЕслИ;
	КонецЦикла;
	//Если есть Тогда
	//	ТЦ = Справочники.ТипыЦенНоменклатуры.Выбрать();
	//	Пока ТЦ.Следующий() Цикл
	//		ЗапросНашаНоменклатура = Новый Запрос;
	//		ЗапросНашаНоменклатура.Текст = "ВЫБРАТЬ
	...
	//		|				И ТипЦен = &ТЦ
	//		|				И Поставщик = &Пост) КАК ЦеныНоменклатурыСрезПоследних";
	//		ЗапросНашаНоменклатура.УстановитьПараметр("СписНом",СписНомБезОст);
	//		ЗапросНашаНоменклатура.УстановитьПараметр("ТЦ",ТЦ.Ссылка);
	//		ЗапросНашаНоменклатура.УстановитьПараметр("Пост",Константы.ОсновнойСклад.Получить());
	//		Рез = ЗапросНашаНоменклатура.Выполнить().Выбрать();
	//		Пока Рез.Следующий() Цикл
	//			 ИзменитьЦену(Рез.Номенклатура,Тц.Ссылка);
	//		КонецЦикла;
	//	КонецЦикла;
	//КонецЕслИ;
	
	
	Если Отменен Тогда  //fofa
...
//с.д.(
Процедура ЗаполнитьНП_APIКлюч() Экспорт
	ТолькоАктивныеОтправители = ?(РольДоступна("ВыборВсехОтправителейНП_СД"),Ложь,Истина);
	НП_APIКлюч = МодульНовойПошты_СД.ПолучитьAPIКлючПоДате(Дата,ТолькоАктивныеОтправители);
	Если не НП_APIКлюч = "" Тогда
		...
		Если Не НастройкиAPIКлюча = Неопределено Тогда
			НП_Отправитель = НастройкиAPIКлюча.НП_Отправитель;
			НП_ГородОтправителя = НастройкиAPIКлюча.НП_ГородОтправителя;
			НП_ОтделениеОтправки = НастройкиAPIКлюча.НП_ОтделениеОтправки;
			НП_КонтактОтправителя = НастройкиAPIКлюча.НП_КонтактОтправителя;
			НП_ТелефонОтправителя = НастройкиAPIКлюча.НП_ТелефонОтправителя;
			НП_Получатель = НастройкиAPIКлюча.НП_КонтрагентПолучатель;
			НП_ФормаОплаты = НастройкиAPIКлюча.НП_ФормаОплаты;
			НП_ТипПлательщика = НастройкиAPIКлюча.НП_ТипПлательщика;
			НП_ТипДоставки = НастройкиAPIКлюча.НП_ТипДоставки;
			НП_ТипГруза = НастройкиAPIКлюча.НП_ТипГруза;
			НП_ОписаниеГруза = НастройкиAPIКлюча.НП_ОписаниеГруза;
			НП_КоличествоМест = 0;
			НП_Вес = 1;
			НП_КонтрольОплаты = НастройкиAPIКлюча.НП_КонтрольОплаты;
			
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СкладГруппа) Тогда
		СкладГруппа = Справочники.Склады.НайтиПоКоду("022");
	КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	НП_ТТН = "";
	НП_МаркировкаНапечатана = Ложь;
	Wayforpay_url_INVOICE = "";
	ТекстДляОтправкиСМС = "";
	id_СМС_СД = "";
	Статус_СМС = "";  
	MagentoID ="";
...
Процедура ТоварыСуммаПоУсловиюПоставкиПриИзмененииОб(ТекущиеДанные) Экспорт
	//+БП: d.matyash (16.09.2024) <>          
 
	ТекущиеДанные.БП_Маржа = ТекущиеДанные.Сумма - ТекущиеДанные.БП_СуммаПоУсловиюПоставки;
	Если ТекущиеДанные.Сумма > 0 тогда
		ТекущиеДанные.БП_ПроцентСкидки = ТекущиеДанные.БП_Маржа / ТекущиеДанные.Сумма * 100;
	Иначе
		ТекущиеДанные.БП_ПроцентСкидки = 0;
	КонецЕсли;
	ТекущиеДанные.БП_ЦенаПоУсловиюПоставки = ТекущиеДанные.Цена - (ТекущиеДанные.Цена * (ТекущиеДанные.БП_ПроцентСкидки / 100));
	
	БП_МаржаДокумента = Товары.Итог("БП_Маржа");
	БП_СуммаПоУсловиюПоставкиДокумента = Товары.Итог("БП_СуммаПоУсловиюПоставки");
	
	Если Товары.Количество() тогда
		БП_ПроцентСкидкиДокумента = Товары.Итог("БП_ПроцентСкидки") / Товары.Количество();
	Иначе   
		БП_ПроцентСкидкиДокумента = 0;	             
	КонецЕсли;
