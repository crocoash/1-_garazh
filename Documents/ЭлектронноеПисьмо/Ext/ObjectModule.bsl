...
Процедура ОтправитьПисьмо(ФормаПисьма = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(БП_ЗаказПоставщику) и СтатусПисьма = Перечисления.СтатусыПисем.Отправленное тогда
		Попытка
			ДокОб = БП_ЗаказПоставщику.ПолучитьОбъект();
			ДокОб.БП_СтатусПисьма = Перечисления.БП_СтатусОтправкиСообщения.Отправлено;
			ДокОб.Записать(); 
		Исключение
			Сообщить("Не вдалося встановити документу Замовлення постачальнику статус відправленого листа!");
		КонецПопытки;         
		Попытка
			Форма=БП_ЗаказПоставщику.ПолучитьФорму("ФормаДокумента"); 
			Форма.Прочитать();
		Исключение
			Сообщить("Не вдалося оновити форму документа Замовлення постачальнику!");
		КонецПопытки;
	КонецЕсли;
	//-БП: d.matyash(18.03.2024) <>	
