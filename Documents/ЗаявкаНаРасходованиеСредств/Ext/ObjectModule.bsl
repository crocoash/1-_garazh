...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Состояние = Перечисления.СостоянияОбъектов.Отклонен и Ссылка.Состояние <> Перечисления.СостоянияОбъектов.Отклонен Тогда
		Почта = "";
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	И КонтактнаяИнформация.Вид.Код = ""000000012""";
	Запрос.УстановитьПараметр("Пол",Ответственный);
	Рез = Запрос.Выполнить().Выбрать();
	Пока рез.Следующий() Цикл
		Почта = Рез.Представление;	
	КонецЦикла; 
		Если ЗначениеЗаполнено(Почта) Тогда
			
		
				Тема = "Вашу заявку отклонили ";
				Текст = Строка(Ссылка);
				АдресСервераSMTP = "smtp.i.ua";
				Аутентификация = 1;
				ПользовательSMTP = "1c_8@i.ua";
				ПарольSMTP = "GFHJKM";
				ПортSMTP = 465;
				Отправитель = "Информация  <1c_8@i.ua>";
				
				
				КомуОтображение = Почта;
				...
				//					Адрес1 = "1c_8@i.ua";
				//						Адрес1 = "am@interstone.su";
				
				//Получатель1 = Письмо.Получатели.Добавить(Адрес1);	
				Получатель1 = Письмо.Получатели.Добавить(Почта);	//ПОМЕНЯТЬЬЬЬЬЬЬЬЬЬЬЬЬЬ
				
				Письмо.Отправитель.Адрес = "1c_8@i.ua";
				Письмо.Отправитель.ОтображаемоеИмя = Отправитель;
				Письмо.Тема = Тема;
				...
				Попытка 
					...
					//Предупреждение("Письмо отправлено!",2);
				Исключение
					//		Возврат;
				КонецПопытки;
...
КонецЕсли; 
	...
	Если Состояние = Перечисления.СостоянияОбъектов.Утвержден и Ссылка.Состояние <> Перечисления.СостоянияОбъектов.Утвержден Тогда
		Почта = "";
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		               |	КонтактнаяИнформация.Представление
