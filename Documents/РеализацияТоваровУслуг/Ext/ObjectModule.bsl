...
Функция ПоказатьТелефон() //FOFA
	...
	Запрос.УстановитьПараметр("Об",Контрагент);
...
//	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Шапка.Склад) Тогда
		...
		ОбластьМакета.Параметры.Склад = СокрЛП(Шапка.Склад);
		ТабДокумент.Вывести(ОбластьМакета);
	КонецЕсли;
	...
	если Организация.Код <> "000000001" Тогда
		...
		АвансЗапрос.УстановитьПараметр("Заказ",Сделка);
		...
		Пока Результат.Следующий() Цикл
			Если Результат.СуммаВзаиморасчетовРасход > 0 Тогда
				...
				ОбластьПробел.Параметры.ЗначениеПараметра = Строка(Сделка.СуммаДокумента-Результат.СуммаВзаиморасчетовРасход) + " грн.";
				ТабДокумент.Вывести(ОбластьПробел);
				Прервать; 
			КонецЕсли;
		КонецЦикла;
	...
	//Если Организация = Справочники.Организации.НайтиПоКоду("000000001").Ссылка Тогда
	...
	Для каждого ВыборкаСтрокТовары из ЗапросТовары Цикл 
		...
		КонецЕсли;
		Если Организация = Справочники.Организации.НайтиПоКоду("000000001").Ссылка Тогда                      //FOFA
...
Функция ПечатьАктаОбОказанииУслугРаздел()
	...
	|	РеализацияТоваровУслуг.Содержание КАК Номенклатура,
	|	РеализацияТоваровУслуг.Содержание КАК Товар,
	...
	|	РеализацияТоваровУслуг.Номенклатура.ВидНоменклатуры КАК ВидНоменклатуры,
	|	РеализацияТоваровУслуг.Номенклатура.ВидНоменклатуры.БП_Раздел КАК Раздел
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Услуги КАК РеализацияТоваровУслуг
	|ГДЕ
	|	РеализацияТоваровУслуг.Ссылка = &ТекущийДокумент
	...
	ТЗРазделов = ТаблицаУслуги.Скопировать(,"Раздел"); 
	ТЗРазделов.Свернуть("Раздел",);
	МассивРазделов = ТЗРазделов.ВыгрузитьКолонку("Раздел");
	ТаблицаУслугиВрем = ТаблицаУслуги.СкопироватьКолонки();
	Для каждого Раздел из МассивРазделов цикл  
		Если ЗначениеЗаполнено(Раздел) тогда
			ТаблицаУслугиВрем.Очистить();
			ТаблицаУслугиОтбор = ТаблицаУслуги.НайтиСтроки(Новый Структура("Раздел",Раздел));
			
			Для каждого Стр из ТаблицаУслугиОтбор цикл
				НовСтр = ТаблицаУслугиВрем.Добавить();
				ЗаполнитьЗначенияСвойств(НовСтр,Стр);
			КонецЦикла;
			
			ТаблицаУслугиВрем.Свернуть("Раздел","Сумма,Скидка,СуммаНДС");
			
			Для Каждого СтрокаТабличнойЧасти Из ТаблицаУслугиВрем Цикл	

				НомерСтроки = НомерСтроки + 1;
				
				ОбластьСтроки.Параметры.Заполнить(СтрокаТабличнойЧасти);
				ОбластьСтроки.Параметры.Товар = СтрокаТабличнойЧасти.Раздел;
				ОбластьСтроки.Параметры.НомерСтроки = НомерСтроки;
				ОбластьСтроки.Параметры.Количество = 1;
				ОбластьСтроки.Параметры.ЕдиницаЦены = "послуга";
		...
		Для Каждого СтрокаТабличнойЧасти Из ТаблицаУслуги Цикл	
			Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.раздел) тогда
				Продолжить;
			КонецЕсли;
			НомерСтроки = НомерСтроки + 1;
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "АктРаздел" Тогда
	    ТабДокумент = ПечатьАктаОбОказанииУслугРаздел();
	ИначеЕсли ИмяМакета = "1Т" Или ИмяМакета = "_1Т" Тогда
 	    ТабДокумент = ПечатьТТН();  
	//Цветков<>  
	ИначеЕсли ИмяМакета = "АвтоПечать" Тогда
		ПФ = ОбщегоНазначения.ПолучитьШаблонАвтоПечать(Ложь,Организация, Сделка.ФормаРасчета, Подразделение);
		...
		Перечисления.ВидыДополнительныхВнешнихОбработок.ПечатнаяФорма,ПФ, Неопределено, 1); 
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураМакетов.Вставить("АктРаздел"            , "Акт про надання послуг (по розділам)");
	...
	СтруктураМакетов.Вставить("АвтоПечать" , "Автопечать расходной накладной / реализации покупателю");
...
//		Если СтрокаТаблицы.КоличествоОстатокПоЗаказу = 0 Тогда//fofa
...
//		КонецЕсли; 
	...
	//	Если СтрокаТаблицы.ОстатокНаСкладе <= 0 Тогда//FOFA
	...
	Если ТабличнаяЧасть = Товары Тогда
		Если ПревышатьОстаткиОрганизации Тогда
			ТаблицаБезЦен.Свернуть("Номенклатура,Цена, ХарактеристикаНоменклатуры","КоличествоОстатокПоЗаказу");
		Иначе
			ТаблицаБезЦен.Свернуть("Номенклатура,Цена, ХарактеристикаНоменклатуры, КоличествоОстатокОрганизации","КоличествоОстатокПоЗаказу");
		КонецЕсли;
	Иначе
		ТаблицаБезЦен.Свернуть("Номенклатура,Цена, ХарактеристикаНоменклатуры","КоличествоОстатокПоЗаказу");//FOFA
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы ИЗ ТаблицаБезЦен Цикл
		...
		Если ТабличнаяЧасть = Товары Тогда
			//Если ПревышатьОстаткиОрганизации Тогда
				ОстатокКоличестваПоЗаказу = СтрокаТаблицы.КоличествоОстатокПоЗаказу;//FOFA
			//Иначе
		...
		СтруктураПоиска.Цена = СтрокаТаблицы.Цена;
		Если ТабличнаяЧасть = Товары Тогда
		...
		//	Если СтрокаЗаказа.КоличествоОстатокПоЗаказу <= 0 Тогда//fofa
		//		Продолжить;
		//	Иначе
				...
				Для каждого СтрокаОстатков Из СтрокиОстатков Цикл
					...
					Если Сч = 1 Тогда
						...
						Если ТабличнаяЧасть = Товары
							...
							КолОстДоступно = КолОстДоступно;//fofa
						КонецЕсли;
						
					КонецЕсли;
					
					//Если СтрокаОстатков.ОстатокНаСкладе = 0 Тогда        //fofa
					//	Продолжить;
					//КонецЕсли;
					...
					КоличествоКСписанию   = ОстатокКоличестваПоЗаказу;//FOFA
					КоличествоКЗаполнению = ОстатокКоличестваПоЗаказу;//FOFA
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если ПроводитьПоПартиям Тогда
        Если БП_ПроверкаНаОбработкуПереоценки(Отказ) тогда
			БП_УправлениеЗапасамиПартионныйУчет.ДвижениеПартийТоваров(Ссылка, Движения.СписанныеТовары.Выгрузить());
		КонецЕсли;
		тест = РегистрыНакопления.ПартииТоваровНаСкладах.ПолучитьСебестоимостьТовара(Ссылка);
		УправлениеЗапасамиПартионныйУчет.ДвижениеПартийТоваров(Ссылка, Движения.СписанныеТовары.Выгрузить(), ,,,,Останавливаться);
		тест1 = РегистрыНакопления.ПартииТоваровНаСкладах.ПолучитьСебестоимостьТовара(Ссылка);
		//тест2 = РегистрыНакопления.ПродажиСебестоимость.ПолучитьСебестоимостьТовара(Ссылка);
		  
		Если Останавливаться тогда
			Отказ = Истина;
		КонецЕсли;
	Иначе
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя")
	 ИЛИ ТипЗнч(Основание) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			БП_Марка = Основание.БП_Марка;
			БП_ТипАвто = Основание.БП_ТипАвто;
			БП_Модель = Основание.БП_Модель;
			БП_НомерВинКода = Основание.БП_НомерВинКода;
			БП_НомернойЗнак = Основание.БП_НомернойЗнак;
		КонецЕсли;
		//-БП: d.matyash (03.12.2024) <> 
		...
		//с.д.(
		для каждого Стр из Товары Цикл
			НайденТовар = Основание.Товары.Найти(Стр.Номенклатура);
			Если Не НайденТовар = Неопределено Тогда
				Стр.ДатаПроизводства = НайденТовар.ДатаПроизводства;
				Стр.Страна = НайденТовар.Страна;    
				//+БП: d.matyash (16.09.2024) <> 
				//Если ЗначениеЗаполнено(НайденТовар.БП_ПроцентСкидки) тогда
				//	Стр.Цена = Стр.Цена - (Стр.Цена * (НайденТовар.БП_ПроцентСкидки / 100));
				//	Стр.Сумма = Стр.Сумма - (Стр.Сумма * (НайденТовар.БП_ПроцентСкидки / 100));
				//	Стр.СуммаНДС = Стр.СуммаНДС - (Стр.СуммаНДС * (НайденТовар.БП_ПроцентСкидки / 100));   
...
#КонецЕсли
		...
		КонецЕсли;  
	//С.Д. Сеня 15,05,24 +	
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемТовараНаХранение") или
		      ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТовараСХранения") Тогда  
			  	  
	   Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемТовараНаХранение") Тогда 
		   Основание_ = Основание.Сделка;
	   ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТовараСХранения") Тогда	 
		  Основание_ = Основание.Заказ;
	   КонецЕсли; 
	   дата = ТекущаяДата();
		// Заполним реквизиты из стандартного набора по документу основанию.
		ЗаполнениеДокументов.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание_);

		ЗаполнениеДокументов.ЗаполнитьДанныеДоставкиПоОснованию(ЭтотОбъект, Основание_);
		...
		#Если Клиент Тогда
			Если Не ЗначениеЗаполнено(Склад) И НЕ мУказаниеСкладовВТЧ И ТипЗнч(Основание_) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
				ВвестиЗначение(Склад, "Выберите склад", Тип("СправочникСсылка.Склады"));
			КонецЕсли;
		#КонецЕсли

		Если ТипЗнч(Основание_) = Тип("ДокументСсылка.ЗаказПокупателя")
		 Или (ТипЗнч(Основание_) = Тип("ДокументСсылка.СчетНаОплатуПокупателю")
		      И ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам) Тогда
			Сделка = Основание_;
		Иначе
			Сделка = Основание_.ЗаказПокупателя;
		КонецЕсли;

		Если ТипЗнч(Основание_) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда

			СкопироватьТовары(Основание_);
			СкопироватьВозвратнуюТару(Основание_);
			СкопироватьУслуги(Основание);
						
			Если ТипЗнч(Основание_.СтруктурнаяЕдиница) = Тип("СправочникСсылка.БанковскиеСчета") Тогда
				БанковскийСчетОрганизации = Основание_.СтруктурнаяЕдиница;
			КонецЕсли;
			
		ИначеЕсли Основание_.Проведен Тогда // Заказ покупателя
			
			// Заполним табличные части неполученными ТМЦ (резервом) по заказу покупателя.
			//ЗаполнитьТабличнуюЧастьПоЗаказуПокупателяУпр(Товары, Основание_);
			//ЗаполнитьТабличнуюЧастьПоЗаказуПокупателяУпр(ВозвратнаяТара, Основание_);
			ЗаполнитьУслугиПоОстаткамУпр();
    ...
    Комментарий = Основание.Комментарий;//FOFA     
	ДокументОснование_СД = Основание;//С.Д. Сеня 15,05,24 +
	...
	//+БП: d.matyash (03.12.2024) <>
	БП_ОбщийМодуль.ИзменитьОрганизацию(ЭтотОбъект, Ложь,Истина,Истина);
	//-БП: d.matyash (03.12.2024) <> 
	
	//+БП: d.matyash (04.11.2025) <> 
	БП_ОбщийМодуль.БП_ЗаполнитьПартии(ЭтотОбъект);
	//-БП: d.matyash (04.11.2025) <>
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//FOFA
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение и РольДоступна("Караван") и Склад <> Справочники.Склады.НайтиПоКоду("Г00000003").Ссылка Тогда
		Сообщить("Вам запрещено проводить данные документы");
		Отказ = Истина;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Телефон) Тогда
		Если ЗначениеЗаполнено(Сделка) Тогда
			Телефон = Сделка.Телефон;
		КонецЕсли; 
	КонецЕсли;       
	
	//с.д.(
	Если ЗначениеЗаполнено(Сделка) Тогда
		Если не Сделка.Ответственный = ОтветственныйПоЗаказу_СД Тогда
			ОтветственныйПоЗаказу_СД = Сделка.Ответственный;
		КонецЕсли;
	КонецЕсли;
	//с.д.)    
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	РежимПроведения =РежимПроведенияДокумента.Оперативный;//FOFA	
	Если мУдалятьДвижения Тогда
		ОбщегоНазначения.УдалитьДвиженияРегистратора(ЭтотОбъект, Отказ); 
		Документы.ПереоценкаЗапасов_СД.УдалитьВсе(Ссылка);
	КонецЕсли;
	...
	СтруктураПолей.Вставить("ДокументПартии"              ,"ДокументПартии");   //FOFA
	...
	//	Если ЗначениеЗаполнено(Строка.ДатаПроизводства) или ЗначениеЗаполнено(Строка.Страна) Тогда
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ЗаказПокупателя.ПредЗаказ
	|ИЗ
	|	Документ.ЗаказПокупателя КАК ЗаказПокупателя
	|ГДЕ
	|	ЗаказПокупателя.Проведен = Истина
	|	И ЗаказПокупателя.Ссылка = &Сделка";
	Запрос.УстановитьПараметр("Сделка",Сделка);
	Рез = Запрос.Выполнить().Выбрать();
	Пока Рез.Следующий() Цикл
		Пред = Рез.ПредЗаказ;
	КонецЦикла;
	Если Пред <> Документы.ПредЗаказ.ПустаяСсылка() и Пред <> 0 Тогда
		если Организация.Код = "000000001" Тогда
			//Для Каждого СтрПред из Пред.ДополнительныеУслуги Цикл
			//	если СтрПред.Использовать Тогда
			//		Движение = Движения.ПродажиСебестоимость.Добавить();
			//		Движение.Период = Дата;
			//		Движение.Номенклатура = СтрПред.Номенклатура;
			//		Движение.ЗаказПокупателя = Сделка;
			//		Движение.Подразделение = подразделение;
			//		ДВижение.Количество = СтрПред.Количество;
			//		Движение.Стоимость = Стрпред.СуммаЗакупки;
			...
			Для Каждого Стр из Услуги Цикл
				Для Каждого СтрПред из Пред.ДополнительныеУслуги Цикл
					если СтрПред.Номенклатура = Стр.Номенклатура Тогда
						Движение = Движения.ПродажиСебестоимость.Добавить();
						Движение.Период = Дата;
						Движение.Номенклатура = Стр.Номенклатура;
						Движение.ЗаказПокупателя = Сделка;
	...
	Иначе
		Для Каждого Стр из Услуги Цикл
			Если стр.Номенклатура.Код = "Г0000001886" Тогда
				Себестоимость = 0;
				Диам = 0;
				КвоШин = 0;
				Для Каждого СтрТ из Товары Цикл
					Диам = СтрТ.Номенклатура.ДиаметрДиска;
					КвоШин = СтрТ.Количество;
				КонецЦикла;
				...
				Рез = Запрос.Выполнить().Выбрать();
				Пока Рез.Следующий() Цикл
					Себестоимость = Рез.Закупка;
				КонецЦикла;
				Если КвоШин = 2 Тогда
					Себестоимость = Себестоимость/4*2*1.1;
				ИначеЕсли Стр.Количество = 0.25 Тогда
					Себестоимость = Себестоимость/4*1.2;
				КонецЕслИ;
				Движение = Движения.ПродажиСебестоимость.Добавить();
				Движение.Период = Дата;
				Движение.Номенклатура = Стр.Номенклатура;
				...
				Движение.СписаниеПартий = Истина;
			КонецЕсли;
			Если стр.Номенклатура.Код = "Г0000022193" Тогда
				Себестоимость = 0;
				Диам = 0;
				КвоШин = 0;
				Для Каждого СтрТ из Товары Цикл
			...
			Если стр.Номенклатура.Код = "00000019323" Тогда
				Себестоимость = 0;
				Диам = 0;
				КвоШин = 0;
				Для Каждого СтрТ из Товары Цикл
	...
	Если Не Отказ Тогда
		Если ЗначениеЗаполнено(Сделка) Тогда
				Если Сделка.Статус <>  Справочники.Статусы.НайтиПоКоду("000000037") Тогда
					ЗАк = Сделка.ПолучитьОбъект();
					ЗАк.Статус = Справочники.Статусы.НайтиПоКоду("000000037");
					Зак.Записать();
				КонецЕслИ;
		КонецЕслИ;
	КонецЕслИ;
	если ЗначениеЗаполнено(Сделка)  и Сделка.СуммаБонуса > 0 Тогда
		Движения.Бонусы.Очистить();
		Движение = Движения.Бонусы.Добавить();
		Движение.ВидДвижения = ВидДвиженияНакопления.Приход;
		Движение.Период = Дата;
		Движение.Контрагент = Сделка.ПриведиДруга;
		Движение.Бонус = Сделка.СуммаБонуса;
...
Функция БП_ПроверкаНаОбработкуПереоценки(Отказ)   
	
	Если ТипЗнч(Ссылка.Сделка) <> Тип("ДокументСсылка.ЗаказПокупателя") ИЛИ Отказ тогда
		Возврат Ложь;	
	КонецЕсли;  	
	
	ТЗСкидокПоУсловиюПоставки = Ссылка.Сделка.Товары.выгрузить(,"Номенклатура,БП_ПроцентСкидки,БП_ЦенаПоУсловиюПоставки");

	Для каждого СтрокаТЗ из ТЗСкидокПоУсловиюПоставки цикл
		Если СтрокаТЗ.БП_ПроцентСкидки > 0 И СтрокаТЗ.БП_ЦенаПоУсловиюПоставки > 0 тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;            
