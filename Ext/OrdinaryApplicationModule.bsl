//fofaвова
Перем M301 Экспорт; 
Перем ККРВключен Экспорт;
//_fofa

//Э_NCT(22.09.2015 www.nct.ua 
Перем Э_BinotelPhone Экспорт;
//Э_NCT)
//Э_Волощук (27.09.2019 14:30:51
Перем Э_ФормаСпискаНоменклатуры Экспорт;
//Э_Волощук)
Перем глСерверТО;
Перем мКлиентOnline;

Перем глОбщиеЗначения Экспорт;
Перем глОтбор Экспорт;
перем ТабличкаПереноса Экспорт;
...
Перем ПараметрыЗаказа Экспорт; //fofa
Перем ПодбираемЗаказ Экспорт;
...
Процедура ПриНачалеРаботыСистемы()
	...
	////FOFA
	//Гараж.ПлановыеОстатки();
	
	//Э_NCT(2014.03.25 www.nct.ua Binotel: Код встраивания в произвольную конфигурацию
	Э_ТелефонияBinotelКлиент.Э_ОткрытьПанельБинотел(Э_BinotelPhone);
	//Э_NCT)
	
	//fofaвова
	COM = 1; 
	Если РольДоступна("АдминистраторККМ") Тогда
		Технологии.ПодклККР();  
		Обр = Обработки.ОбслуживаниеКассы.ПолучитьФорму();
		Обр.Открыть();
	КонецЕсли;
	
	//с.д.(
	Если УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(мТекущийПользователь, "СенсорныйЕкран_СД") = Истина Тогда
		ПараметрыСеанса.СенсорныйЕкран_Сд = истина;
		
		Форма_СД = Обработки.СенсорныйЕкран_РабочийСтол_СД.ПолучитьФорму("Форма_РабочийСтол");
	...
	//Цв
	МассивАктивныхМенеджеров = Справочники.Пользователи.ПолучитьМассивМенеджеров(); 
	
	//Для теста
	ЦветковАнтон = Справочники.Пользователи.НайтиПоНаименованию("Галушка Тарас");
	ГалушкоТарас =  Справочники.Пользователи.НайтиПоНаименованию("Антон Цветков"); 
	МассивАктивныхМенеджеров.добавить(ЦветковАнтон); 
	МассивАктивныхМенеджеров.добавить(ГалушкоТарас);	
		
	Если МассивАктивныхМенеджеров.Найти(мТекущийПользователь) <> Неопределено Тогда   
		Справочники.Номенклатура.ПолучитьФормуСписка().Открыть();
		Документы.ЗаказПокупателя.ПолучитьФормуСписка().Открыть();
		Документы.ПредЗаказ.ПолучитьФормуСписка().Открыть();
	КонецЕсли; 
	//Цв
